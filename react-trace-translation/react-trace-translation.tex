...
\text{Type } \tau &::= \one \mid \mf{Bool} \mid \tau \xrightarrow{E} \tau
...
\begin{proof}
Via induction on the derivation of the respective typing judgment.

\emph{Case} $\Gamma, x: \tau \semi \Sigma \vdashv x: \tau'$.
Applying substitution, we want to show 
$\Gamma \semi \Sigma \vdashv v: \tau'$, which is exactly our first premise
(with $\tau'=\tau$).

\emph{Case} $\Gamma, x: \tau \semi \Sigma \vdashv (): \one$.
Trivial, since $()[v/x]=()$ and $\Gamma \semi \Sigma \vdashv (): \one$ axiomatically.
We elide analogous cases for boolean and string constants.

\emph{Case} $\Gamma, x: \tau \semi \Sigma \vdashv \lambda y.e : \tau_1 \rightarrow \tau_2$.
...
gives that $\Gamma, y: \tau_1 \vdashe e[v/x]: \tau_2$.
We conclude $\Gamma \vdashv (\lambda y.e) [v/x]: \tau_1 \rightarrow \tau_2$
by reapplication of \textsc{TVal-Lam}.

\emph{Case} $\Gamma, x: \tau \semi \Sigma \vdashv 
\mf{tag}(v', \num{v_i}, \num{v_j}): \mf{View}$.
...
\emph{Case} $\Gamma, x: \tau \semi \Sigma \vdashe \eret{v'}: \tau'$.
...
\emph{Case} $\Gamma, x: \tau \semi \Sigma \vdashe
\elet{y}{e_1}{e_2}: \tau_2$. Inversion gives 
$\Gamma, x: \tau \semi \Sigma \vdashe e_1: \tau_1$ and in any case,
and by inductive hypothesis, $\Gamma \semi \Sigma \vdashe e_1[v/x] : \tau_1$.
...
giving $\Gamma, x: \tau, y: \tau_1 \vdashe e_2: \tau_2$, and
...
\emph{Case} $\cdot \semi \Sigma \vdashe \eret{v} \semi \tau$.
The theorem holds trivially by case \emph{(i)}.

\emph{Case} $\cdot \semi \Sigma \vdashe \elet{x}{e_1}{e_2}: \tau_2$.
...
\emph{Case} $\cdot \semi \Sigma \vdashe v_1\ v_2: \tau_2$.
...
\emph{Case} $\cdot \semi \Sigma \vdashe \eif{v}{e_1}{e_2} : \tau$.
...
\emph{Case} $\cdot \semi \Sigma \vdashe \eset{v_1}{v_2}: \one$.
...
\begin{proof}
By induction on derivation of \emph{(iii)}, $\sigma \semi e \mapsto e' \semi \sigma'$.

\emph{Case} $\sigma \semi \elet{x}{e_1}{e_2} \mapsto \elet{x}{e_1'}{e_2} \semi \sigma'$.
By inversion on \textsc{E-Let}, we have $\sigma \semi e_1 \mapsto e_1' \semi \sigma'$.
...
\emph{Case} $\sigma \semi \elet{x}{\eret{v}}{e} \mapsto e[v/x] \semi \sigma$.
...
\emph{Case} $\sigma \semi (\lambda x: \tau_1.e)v \mapsto e[v/x] \semi \sigma$.
Inversion on \emph{(i)} gives $\Gamma, x: \tau_1 \semi \Sigma \vdashe e: \tau$
and $\Gamma \semi \Sigma \vdashv v: \tau_1$. We again get
$\Gamma \semi \Sigma \vdashe e[v/x]: \tau$ via the substitution lemma.

\emph{Case} $\sigma \semi \eif{\mf{true}}{e_1}{e_2} \mapsto e_1 \semi \sigma$.
Immediate by inversion on \emph{(i)}. Case for \textsc{E-IfFalse} is analogous.

\emph{Case} $\sigma \semi \eref{v} \mapsto \eret{\ell} \semi \sigma[\ell \mapsto v]$
(for $\ell$ fresh).
...
\emph{Case} $\sigma \semi \eget{\ell} \mapsto \eret{\sigma[\ell]} \semi \sigma$.
...
\emph{Case} $\sigma \semi \eset{\ell}{v} \mapsto \eret{()} \semi \sigma[\ell \mapsto v]$.
...
\begin{proof}
Follows from progess \& preservation.
\end{proof}
...
\begin{align*}
&\llrr{e} = \lambda \_: \one.\llrr{e}\\
&\llrr{\estate{x}{\ell}{v}{\hat{e}}} =\\
&\elet{x}{\eref{\llrr{v}}}{
  \elet{x_\mf{set}}{\lambda y: ?.\eset{x}{y}}{\llrr{\hat{e}}}
}
\end{align*}
Programs:
\begin{align*}
\llrr{\erun{e}} &= \llrr{e}\\
\llrr{\elet{\mc{C}(x: \tau)}{\hat{e}}{P}} &= ?
\end{align*}

Current issues:
\begin{itemize}
\item Translating mutually dependent components
requires mutual recursion in target language
\begin{enumerate}
    \item Remove mutual dependence from source (would probably also terminate)
    \item Add mutual recursion to target only (straightforward)
    \item Add mutual recursion to both (could maybe simplify source)
\end{enumerate}
\item useState intializes a reference,
but we treat this as an (unqualified) variable in use
\begin{itemize}
    \item Solution: (capture-avoiding) substitute $\eget{x}/x$ 
    in rest of expression
    \item (how to express this idea?)
\end{itemize}
\end{itemize}
