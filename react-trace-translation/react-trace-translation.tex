...
\newcommand{\Pstep}{\Mapsto^{\mf{Prog}}}
\newcommand{\bsinit}{\Downarrow^{\mf{Init}}}
\newcommand{\bssucc}{\Downarrow^{\mf{Succ}}}
...
Judgment: $\mc{D} \vdash v \bsinit \pair{t}{m}$.
\begin{mathpar}
...
Judgment: $\mc{D} \semi m \vdash t \bssucc m'$

\begin{mathpar}
\inferrule*[right=Check-Lit] { } {
  \mc{D} \semi m \vdash \mf{lit}\ v \bssucc \cdot
}
\and
\inferrule*[right=Check-Node] {
  (0 \leq j < n) \quad
  \mc{D} \semi m \vdash t_j \bssucc m_j
} {
  \mc{D} \semi m \vdash \mf{node}(id, v, \num{v_i}, \num{t_j}) 
  \bssucc \bigcup_{\forall j} m_j
}
\and
\inferrule*[right=\hl{Check-PathConst}] {
  m[p].\mf{rerender} = \mf{ff}
  \and
  \mc{D} \semi m \vdash m[p].\mf{child} \bssucc m'
  = m
} {
  \mc{D} \semi m \vdash p \bssucc m'
}
\and
\inferrule*[right=\hl{Check-PathDiff}] { 
  ...
  \\
  \mc{D} \semi m \vdash v' \bsinit \pair{t}{m'}
} {
  \mc{D} \semi m \vdash p \bssucc m' \mid 
...
\and
\inferrule*[right=\hl{Loop-HandlePop}] { } {
...
\item $\Gamma \semi \Pi \vdmwk m$, and
...
If $\mc{D} \vdash v \bsinit \pair{t}{m}$,
...
\bsinit
\pair{\mf{node}(\mf{id}, v, \num{v_i}, \num{t_j})}{\bigcup_j m_j}$.
\item For all $j$, we also have $\mc{D} \vdash v_j \bsinit \pair{t_j}{m_j}$.
...
\bsinit \pair{p}{m' \cup (m \mid m[p].\mf{child} = t)}$.
...
\begin{enumerate}[(a)]
  ...
  \item $\mc{D} \vdash v \bsinit \pair{t}{m'}$.
\end{enumerate}
...
\item We have $\mc{D} \vdash \mf{text}\ v \bsinit \pair{\mf{lit}\ v}{\cdot}$.
...
\item We have $\mc{D} \vdash \mf{tag}(v, \num{v_i}, \num{v_j})
  \bsinit
  \pair{\mf{node}(\mf{id}, v, \num{v_i}, \num{t_j})}{\bigcup_j m_j}$.
\item Furthermore, for all $j$, we have (a) that $\mc{D} \vdash v_j \bsinit \pair{t_j}{m_j}$.
