\documentclass{article}
\usepackage[utf8]{inputenc}

\usepackage{mathpartir}
\usepackage{amsmath}
\usepackage{amsfonts}
\usepackage{amssymb}
\usepackage{amsthm}
\usepackage{listings}
\usepackage{stmaryrd}
\usepackage{parskip}
\usepackage{mathtools}
\usepackage{xcolor}
\usepackage{enumerate}

\newcommand{\num}[1]{\overline{#1}}
\newcommand{\mf}[1]{\mathsf{#1}}
\newcommand{\mc}[1]{\mathcal{#1}}
\newcommand{\mb}[1]{\mathbf{#1}}
\newcommand{\tf}[1]{\textsf{#1}}
\newcommand{\ts}[1]{\textsc{#1}}
\newcommand{\pp}{+\!\!\!+\: }
\newcommand{\semi}{\,;\,}
\newcommand{\dom}{\mf{dom}}
\newcommand{\po}{\sqsubseteq}

\newcommand{\mathhl}[1]{\colorbox{yellow}{$#1$}}
\newcommand{\hl}[1]{\colorbox{yellow}{#1}}
\newcommand{\grey}[1]{\colorbox{lightgray}{#1}}
\newcommand{\mathgrey}[1]{\colorbox{lightgray}{$#1$}}
\newcommand{\llrr}[1]{\llbracket #1 \rrbracket}

\newcommand{\pair}[2]{\langle #1, #2 \rangle}
\newcommand{\trip}[3]{\langle #1, #2, #3 \rangle}
\newcommand{\four}[4]{\langle #1, #2, #3, #4 \rangle}
\newcommand{\map}[2]{[\num{#1 \mapsto #2}]}
\newcommand{\multistep}[2]{\xmapsto{*}^{#1}_{#2}}
\newcommand{\vdashv}{\vdash_v}
\newcommand{\vdashe}{\vdash_e}

\newcommand{\vdval}{\vdash_\Delta}
\newcommand{\vdexp}{\vdash_\Delta}
\newcommand{\vdtop}{\vdash^{\mf{Top}}_\Delta}
\newcommand{\vdprog}{\vdash^{\mf{Prog}}_\Delta}
\newcommand{\vdtree}{\vdash^{\mf{Tree}}_\Delta}
\newcommand{\vdmwk}{\vDash^{\mf{wk}}_\Delta}
\newcommand{\vdmstr}{\vDash^{\mf{str}}_\Delta}

\newcommand{\topstep}[2]{\mapsto^{#1}_{#2}}
\newcommand{\Topstep}[2]{\Mapsto^{#1}_{#2}}
\newcommand{\pstep}{\mapsto^{\mf{Prog}}}
\newcommand{\Pstep}{\Mapsto^{\mf{Prog}}}
\newcommand{\init}{\Downarrow^{\mf{Init}}}

\newcommand{\one}{\mb{1}}
\newcommand{\elet}[3]{\mf{let}\ #1 = #2\ \mf{in}\ #3}
\newcommand{\eletrec}[3]{\mf{let}\ \mf{rec}\ #1 = #2\ \mf{in}\ #3}
\newcommand{\erun}[1]{\mf{run}\ #1}
\newcommand{\eif}[3]{\mf{if}\ #1\ \mf{then}\ #2\ \mf{else}\ #3}
\newcommand{\eret}[1]{\mf{return}\ #1}
\newcommand{\estate}[4]{\mf{let}\ (#1, #1_{\mf{set}}) = \mf{useState}^{#2}(#3)\ \mf{in}\ #4}
\newcommand{\vset}[2]{\mf{set}@^{#1}_{#2}}
\newcommand{\eref}[1]{\mf{ref}(#1)}
\newcommand{\eget}[1]{\ !#1}
\newcommand{\eset}[2]{#1 := #2}
\newcommand{\vrec}[4]{\mf{rec}\ #1(#2 : #3).#4}
\newcommand{\vcomp}[4]{\mf{comp}\ #1(#2 : #3).#4}
\newcommand{\lits}{\{\mf{Bool}, \mf{String}\}}

\newtheorem{theorem}{Theorem}
\newtheorem{lemma}[theorem]{Lemma}
\newtheorem{corollary}[theorem]{Corollary}
\newtheorem{proposition}[theorem]{Proposition}
\newtheorem{postulate}[theorem]{Postulate}

\title{React-tRanslation}
\author{Toby Ueno}

\begin{document}

\maketitle

\section{React-tRace Core}

\subsection{Syntax}

Constructs which only appear in intermediate stages 
of evaluation are in \grey{grey}.
Other non-standard constructs are \hl{highlighted}.
\begin{align*}
\text{Value } v &::= x \mid () \mid \mf{true} \mid \mf{false} 
\mid s \mid \lambda x: \tau.e\\
&\mid \mathhl{\mc{C}} \mid \mathhl{\mf{text}\ v} \mid \mathhl{\mf{tag}(v, \num{v}, \num{v})}
\mid \mathhl{\mf{attr}(v, v)} \mid \mathhl{\mf{onClick}\ v}\\
&\mid \mathgrey{\pair{\mc{C}}{v}} \mid \mathgrey{\vset{\ell}{p}}\\
\text{Expression } e &::= \eret{v} \mid \elet{x}{e}{e} \mid v\ v \mid \eif{v}{e}{e}\\
\text{Top-level expr }\hat{e} &::= e \mid \mathhl{\estate{x}{\ell}{v}{\hat{e}}} \mid \elet{x}{e}{\hat{e}}\\
\text{Program } P &::= e \mid \elet{x}{e}{P} \mid \mathhl{\elet{\mc{C}(x : \tau)}{\hat{e}}{P}}\\
\text{Type } \tau &::= \one \mid \mf{Bool} \mid \mf{String} \mid \tau \xrightarrow{E} \tau
\mid \mathhl{\mf{View}} \mid \mathhl{\mf{Attr}}\\
&\mid \mathhl{\mf{Component}\ \tau} \mid \mathhl{\mf{Setter}\ \tau}\\
\text{Effect } E &::= \epsilon \mid \mathhl{\mf{Mut}}
\end{align*}

\emph{\textbf{Map Ordering}}. Define partial order $\po$ on maps $M=\map{x}{y}$
(e.g. $\Gamma$, $\Pi$, etc.) such that $A_1 \po A_2$ if, for all $x \in \dom(A_1)$,
$A_1[x] = A_2[x]$. (Implicitly, $\dom(A_1) \subseteq \dom(A_2)$.)

\emph{\textbf{Expression Labels}}. Define recursive function $\mc{L}$
on top-level expressions as:
\begin{align*}
&\mc{L}(e) = \emptyset\\
&\mc{L}(\elet{x}{e}{\hat{e}}) = \mc{L}(\hat{e})\\
&\mc{L}(\estate{x}{\ell}{v}{\hat{e}}) = \{\ell\} \cup \mc{L}(\hat{e})
\end{align*}

\emph{\textbf{Internal Representations}}.
\begin{align*}
\text{Type context }\Gamma &::= \map{x}{\tau}\\
\text{Path context }\Pi &::= \map{p}{\mc{C}}\\
\text{Component context }\Delta &::= \map{\mc{C}}{\pair{\tau}{\map{\ell}{\tau}}}\\
\text{Phase }\phi &::= \mf{Init} \mid \mf{Succ}\\
\text{Tree }t &::= \mf{lit}\ v \mid \mf{node}(\mf{id}, v, \num{v}, \num{t}) \mid p\\
\text{Realized component }\pi &::= \{\mf{spec}: \pair{\mc{C}}{v}, \mf{st}: \map{\ell}{v}, \mf{child}: t\}\\
\text{Tree memory }m &::= \map{p}{\pi}\\
\text{Component definitions }\mc{D} &::= \map{\mc{C}}{x.\hat{e}}\\
\text{Mode }\mu &::= \mf{start}(P) \mid \mf{idle}
\mid \mf{handle}(id) \mid \mf{rerender}(m)
\end{align*}

We overload indexing notation on $\Delta$ for brevity,
i.e. $\Delta[\mc{C}]=\tau$ and $\Delta[\mc{C}][\ell]=\tau$.

\subsection{User-Facing Typing}

Names of noteworthy rules are \hl{highlighted}.
Rules for typing internal representations,
i.e. that are necessary for soundness but not typechecking,
are \grey{greyed}. 
Similarly, $\Pi$ is unnecessary for typechecking.

\subsubsection{Value Typing}

Judgment: $\Gamma \semi \Pi \vdval v: \tau$

\begin{mathpar}
\inferrule*[right=TVal-Var] {
  \Gamma[x] = \tau
} {
  \Gamma \semi \Pi \vdval x: \tau
}
\and
\inferrule*[right=TVal-Unit] { } {
  \Gamma \semi \Pi \vdval () : \one
}
\and
\inferrule*[right=TVal-True] { } {
  \Gamma \semi \Pi \vdval \mf{true} : \mf{Bool}
}
\and
\inferrule*[right=TVal-False] { } {
  \Gamma \semi \Pi \vdval \mf{false} : \mf{Bool}
}
\and
\inferrule*[right=TVal-Str] { } {
  \Gamma \semi \Pi \vdval s : \mf{String}
}
\and
\inferrule*[right=TVal-Lam] {
  \Gamma[x \mapsto \tau_1] \semi \Pi \vdexp e : \tau_2 \mid E
} {
  \Gamma \semi \Pi 
  \vdval
  \lambda x: \tau_1.e: \tau_1 \xrightarrow{E} \tau_2
}
\and
\inferrule*[right=\hl{TVal-Comp}] {
  \Delta[\mc{C}] = \tau
} {
  \Gamma \semi \Pi \vdval \mc{C} : \mf{Component}\ \tau
}
\and
\inferrule*[right=\hl{TVal-Text}] {
  \Gamma \semi \Pi \vdval v: \tau
  \and
  \tau \in \lits
} {
  \Gamma \semi \Pi \vdval \mf{text}\ v: \mf{View}
}
\and
\inferrule*[right=\hl{TVal-Tag}] {
  \Gamma \semi \Pi \vdval v: \mf{String}
  \and
  (\forall i)\ \Gamma \semi \Pi \vdval v_i: \mf{Attr}
  \\
  (\forall j)\ \Gamma \semi \Pi \vdval v_j: \mf{View}
} {
  \Gamma \semi \Pi \vdval \mf{tag}(v, \num{v_i}, \num{v_j}): \mf{View}
}
\and
\inferrule*[right=\hl{TVal-Attr}] {
  \Gamma \semi \Pi \vdval v_k: \mf{String}
  \and
  \Gamma \semi \Pi \vdval v_v: \mf{String}
} {
  \Gamma \semi \Pi \vdval \mf{attr}(v_k, v_v): \mf{Attr}
}
\and
\inferrule*[right=\hl{TVal-OnClick}] {
  \Gamma \vdval v: \one \xrightarrow{E} \one
} {
  \Gamma \vdval \mf{onClick}\ v: \mf{Attr}
}
\and
\inferrule*[right=\grey{TVal-Spec}] {
  \Gamma \semi \Pi \vdval \mc{C}: \mf{Component}\ \tau
  \and
  \Gamma \semi \Pi \vdval v: \tau
} {
  \Gamma \semi \Pi \vdval \pair{\mc{C}}{v}: \mf{View}
}
\and
\inferrule*[right=\grey{TVal-Setter}] {
  \Delta[\Pi[p]][\ell] = \tau
} {
  \Gamma \semi \Pi \vdval \vset{\ell}{p} : \mf{Setter}\ \tau
}
\end{mathpar}

\subsubsection{Expression Typing}

Judgment: $\Gamma \semi \Pi \vdexp e : x \mid E$

\begin{mathpar}
\inferrule*[right=TExp-Ret] {
  \Gamma \semi \Pi \vdval v: \tau
} {
  \Gamma \semi \Pi \vdexp \eret{v}: \tau \mid \epsilon
}
\and
\inferrule*[right=TExp-Let] {
  \Gamma \semi \Pi \vdexp e_1 : \tau_1 \mid E
  \and
  \Gamma[x \mapsto \tau_1] \semi \Pi \vdexp e_2 : \tau_2 \mid E
} {
  \Gamma \vdexp \elet{x}{e_1}{e_2}: \tau_2 \mid E
}
\and
\inferrule*[right=TExp-AppFun] {
  \Gamma \semi \Pi \vdval v_1 : \tau_1 \xrightarrow{E} \tau_2
  \and
  \Gamma \semi \Pi \vdval v_2 : \tau_1
} {
  \Gamma \vdexp v_1\ v_2: \tau_2 \mid E
}
\and
\inferrule*[right=\hl{TExp-AppComp}] {
  \Gamma \semi \Pi \vdval v_1 : \mf{Component}\ \tau
  \and
  \Gamma \semi \Pi \vdval v_2 : \tau
} {
  \Gamma \semi \Pi \vdexp v_1\ v_2: \mf{View} \mid \epsilon
}
\and
\inferrule*[right=\hl{TExp-AppSet}] {
  \Gamma \semi \Pi \vdval v_1 : \mf{Setter}\ \tau
  \and
  \Gamma \semi \Pi \vdval v_2 : \tau
} {
  \Gamma \semi \Pi \vdexp v_1\ v_2: \one \mid \mf{Mut}
}
\and
\inferrule*[right=TExp-If] {
  \Gamma \semi \Pi \vdval v : \mf{Bool}
  \and
  \Gamma \semi \Pi \vdexp e_1 : \tau \mid E
  \and
  \Gamma \semi \Pi \vdexp e_2 : \tau \mid E
} {
  \Gamma \vdexp \eif{v}{e_1}{e_2} : \tau \mid E
}
\and
\inferrule*[right=\hl{TExp-Sub}] {
  \Gamma \semi \Pi \vdexp e: \tau \mid \epsilon
} {
  \Gamma \semi \Pi \vdexp e: \tau \mid \mf{Mut}
}
\end{mathpar}

\subsubsection{Top-Level Expression Typing}

Judgment: $\Gamma \semi \Pi \semi \mc{C} \vdtop \hat{e}$

\begin{mathpar}
\inferrule*[right=THat-Lift] {
  \Gamma \semi \Pi \vdexp e: \mf{View} \mid \epsilon
} {
  \Gamma \semi \Pi \semi \mc{C} \vdtop e
}
\and
\inferrule*[right=THat-Let] {
  \Gamma \semi \Pi \vdexp e: \tau_1 \mid \epsilon
  \and
  \Gamma[x \mapsto \tau_1] \semi \Pi \semi \mc{C} \vdtop \hat{e}
} {
  \Gamma \semi \Pi \semi \mc{C} \vdtop \elet{x}{e}{\hat{e}}
}
\and
\inferrule*[right=\hl{THat-State}] {
  \Delta[\mc{C}][\ell] = \tau_1
  \and
  \Gamma \semi \Pi \vdval v: \tau_1
  \\
  \Gamma[x \mapsto \tau_1, x_\mf{set} \mapsto \mf{Setter}\ \tau_1]
  \semi \Pi \semi \mc{C} \vdtop \hat{e}
} {
  \Gamma \semi \Pi \semi \mc{C} \vdtop \estate{x}{\ell}{v}{\hat{e}}
}
\end{mathpar}

\subsubsection{Program Typing}

Judgment: $\Gamma \vdprog P$

\begin{mathpar}
\inferrule*[right=TProg-Lift] {
  \Gamma \semi \cdot \vdexp {e}: \mf{View} \mid \epsilon
} {
  \Gamma \vdprog e
}
\and
\inferrule*[right=TProg-Let] {
  \Gamma \semi \cdot \vdexp e: \tau \mid \epsilon
  \and 
  \Gamma[x \mapsto \tau] \vdprog P
} {
  \Gamma \semi \cdot \vdprog \elet{x}{e}{P}
}
\and
\inferrule*[right=\hl{TProg-Comp}] {
  \Delta[\mc{C}] = \tau
  \and
  \Gamma[x \mapsto \tau] \semi \cdot \semi \mc{C} \vdtop \hat{e}
  \and
  \Gamma \vdprog P
} {
  \Gamma \vdprog \elet{\mc{C}(x: \tau)}{\hat{e}}{P}
}
\end{mathpar}

\subsection{Runtime Typing}

\subsubsection{Trees}

Judgment: $\Pi \vdtree t$
\begin{mathpar}
\inferrule*[right=TTree-Lit] {
  \cdot \semi \Pi \vdval v: \tau
  \and
  \tau \in \lits
} {
  \Pi \vdtree \mf{lit}\ v
}
\and
\inferrule*[right=TTree-Path] {
  p \in \dom(\Pi)
} {
  \Pi \vdtree p
}
\and
\inferrule*[right=TTree-Node] {
  \mf{id\ fresh}
  \and
  \cdot \semi \Pi \vdval v: \mf{String}
  \\
  (\forall i)\ \cdot \semi \Pi \vdval v_i: \mf{Attr}
  \and
  (\forall j)\ \Pi \vdtree t_j
} {
  \Pi \vdtree \mf{node}(\mf{id}, v, \num{v_i}, \num{t_j})
}
\end{mathpar}

\subsubsection{Component Context}
Define $\Delta \vDash \mc{D}$ to say that,
for all $\mc{C} \in \dom(\mc{D})$
(such that $\mc{D}[\mc{C}] = x.\hat{e}$),
we have $[x \mapsto \Delta[\mc{C}]] \semi \cdot \semi \mc{C} \vdtop \hat{e}$.
(Implicitly, $\dom(\mc{D}) \subseteq \dom(\Delta)$).

\subsubsection{Memory}
We have \emph{weak} and \emph{strong} notions of a well-typed memory $m$.

\textbf{\emph{Weak memory typing}}:
We say $\Gamma \semi \Pi \vdmwk m$ if, for all $p \in \dom(m)$,
\begin{itemize}
\item for $m[p].\mf{spec}=\pair{\mc{C}}{v}$,
$\Pi[p] = \mc{C}$ and $\Gamma \semi \Pi \vdval v: \Delta[\mc{C}]$, and
\item for all $\ell$ in $m[p].\mf{st}$,
$\Gamma \semi \Pi \vdval m[p].\mf{st}[\ell] : \Delta[\mc{C}][\ell]$.
\end{itemize}

\textbf{\emph{Strong memory typing}}: 
We say $\Gamma \semi \Pi \semi \mc{D} \vdmstr m$
if:
\begin{itemize}
\item $\Gamma \semi \Pi \vdmwk m$,
\item $\Delta \vDash \mc{D}$, and
\item for all $p \in \dom(m)$,
\begin{itemize}
    \item for $\mc{D}[\Pi[p]]=x.\hat{e}$, 
    $\mc{L}(\hat{e}) \subseteq \dom(m[p].\mf{st})$, and
    \item $\Pi \vdtree m[p].\mf{child}$.
\end{itemize}
\end{itemize}

\subsubsection{Modes, Configurations}: To-do.

\subsection{Program Semantics}

Throughout the semantics, we use the notation $m \mid x = v$
to concisely denote nested map/record updates.
The above should be read as ``$m$ with field $x$ set to $v$''.
For instance, $m \mid m[p].\mf{st}[\ell] = v$ updates the
state field of $m$ at path $p$ and location $\ell$ to value $v$.

Again, we \hl{highlight} interesting rules.

\subsubsection{Expression Semantics}

Judgment: $m \semi e \mapsto e' \semi m'$
\begin{mathpar}
\inferrule*[right=E-Let] {
  m \semi e_1 \mapsto e_1' \semi m'
} {
  m \semi \elet{x}{e_1}{e_2} \mapsto \elet{x}{e_1'}{e_2} \semi m'
}
\end{mathpar}
\begin{align*}
m \semi \elet{x}{\eret{v}}{e} &\mapsto e[v/x] \semi m
&&\textsc{E-Ret}\\
m \semi (\lambda x.e) v &\mapsto e[v/x] \semi m
&&\textsc{E-AppFun}\\
m \semi \mc{C}\ v &\mapsto \eret{\pair{\mc{C}}{v}} \semi m
&&\textsc{\hl{E-AppComp}}\\
m \semi \vset{\ell}{p}\ v &\mapsto \eret{()} \semi m \mid m[p].\mf{st}[\ell] = v
&&\textsc{\hl{E-AppSet}}\\
m \semi \eif{\mf{true}}{e_1}{e_2} &\mapsto e_1 \semi m
&&\textsc{E-IfTrue}\\
m \semi \eif{\mf{false}}{e_1}{e_2} &\mapsto e_2 \semi m
&&\textsc{E-IfFalse}
\end{align*}

\subsubsection{Top-Level Expression Semantics}

Judgment: $m \semi \hat{e} \topstep{\phi}{p} \hat{e}' \semi m'$
\begin{mathpar}
\inferrule*[right=E-Lift] {
  \cdot \semi e \mapsto e' \semi \cdot
} {
  m \semi e \topstep{\phi}{p} e' \semi m
}
\and
\inferrule*[right=E-LetTop] {
  \cdot \semi e \mapsto e' \semi \cdot
} {
  m \semi \elet{x}{e}{\hat{e}} \topstep{\phi}{p} \elet{x}{e'}{\hat{e}} \semi m
}
\end{mathpar}
\begin{align*}
m \semi \elet{x}{\eret{v}}{\hat{e}} &\topstep{\phi}{p} \hat{e}[v/x] \semi m
&&\textsc{E-RetTop}\\
m \semi \estate{x}{\ell}{v}{\hat{e}}
&\topstep{\mf{Init}}{p}
\hat{e}[v/x, \vset{\ell}{p}/x_\mf{set}] \semi m'
&&\textsc{\hl{E-StInit}}\\
(m' &= m \mid m[p].\mf{st}[\ell] = v)\\
m \semi \estate{x}{\ell}{v}{\hat{e}}
&\topstep{\mf{Succ}}{p}
\hat{e}[v'/x, \vset{\ell}{p}/x_\mf{set}] \semi m
&&\textsc{\hl{E-StSucc}}\\
(v' &= m[p].\mf{st}[\ell])
\end{align*}

\subsubsection{Program Expression Semantics}

Judgment: $\mc{D} \semi P \pstep P' \semi \mc{D}'$
\begin{mathpar}
\inferrule*[right=P-Lift] {
  \cdot \semi e \mapsto e' \semi \cdot
} {
  \mc{D} \semi e \pstep e \semi \mc{D}
}
\and
\inferrule*[right=P-Let] {
  \cdot \semi e \mapsto e' \semi \cdot
} {
  \mc{D} \semi \elet{x}{e}{P}
  \pstep
  \elet{x}{e'}{P} \semi \mc{D}
}
\end{mathpar}
\begin{align*}
\mc{D} \semi \elet{x}{\eret{v}}{P}
&\pstep
P[v/x] \semi \mc{D}
&&\textsc{E-RetProg}\\
\mc{D} \semi \elet{\mc{C}(x)}{\hat{e}}{P}
&\pstep
P \semi \mc{D}[\mc{C} \mapsto x.\hat{e}]
&&\textsc{\hl{E-CompDef}}
\end{align*}

\subsection{Event-Loop Semantics}

Say $m \semi \hat{e} \Topstep{\phi}{p} v \semi m'$ 
if $m \semi \hat{e} \xmapsto{*}^{\phi}_{p} \eret{v} \semi m'$
and $m' \semi \hat{e}' \not \topstep{\phi}{p}$.

\subsubsection{Initialization}

Judgment: $\mc{D} \vdash v \init \pair{t}{m}$.
\begin{mathpar}
\inferrule*[right=\textsc{Init-Lit}] { } {
  \mc{D} \vdash \mf{text}\ v 
  \init
  \pair{\mf{lit}\ v}{\cdot}
}
\and
\inferrule*[right=\textsc{Init-Node}] {
  \mf{id\ fresh}
  \and
  (\forall j)
  \quad
  \mc{D} \vdash v_j
  \init
  \pair{t_j}{m_j}
} {
  \mc{D} \vdash \mf{tag}(v, \num{v_i}, \num{v_j})
  \init
  \pair{\mf{node}(\mf{id}, v, \num{v_i}, \num{t_j})}{\bigcup_j m_j}
}
\and
\inferrule*[right=\textsc{Init-Comp}] {
  p\ \mf{fresh}
  \and
  \mc{D}[\mc{C}] = x.\hat{e}
  \\
  [p \mapsto \{\mf{spec}: \pair{\mc{C}}{v}, \mf{st}: \emptyset, \mf{child}: \emptyset\}] 
  \semi \hat{e}[v/x] \Topstep{\mf{Init}}{p} v' \semi m 
  \\
  \mc{D} \vdash v' \Downarrow^{\mf{Init}} \pair{t}{m'}
} {
  \mc{D} \vdash \pair{\mc{C}}{v}
  \init
  \pair{p}{m' \uplus (m \mid m[p].\mf{child} = t)}
}
\end{mathpar}

\subsubsection{Rerendering}

(Here on down in the semantics has yet to be reformulated, as of 15 Dec.)

Judgment: $\mc{D} \semi m_\mf{old} \vdash \mf{check}(m_\mf{new}, t) = m$

\begin{mathpar}
\inferrule*[right=Check-Lit] { } {
  \mc{D} \semi m_\mf{old} \vdash \mf{check}(m_\mf{new}, \mf{lit}\ v) 
  = m_\mf{new}
}
\and
\inferrule*[right=Check-Node] {
  (0 \leq j < n) \quad
  \mc{D} \semi m_\mf{old} \vdash \mf{check}(m_j, t_j) = \pair{t_j'}{m_{j+1}}
} {
  \mc{D} \semi m_\mf{old} \vdash \mf{check}(m_0, \mf{node}(id, v, \num{v_i}, \num{t_j})) = m_n
}
\and
\inferrule*[right=\hl{Check-PathConst}] {
  m_\mf{old}[p].\mf{st} = m_\mf{new}[p].\mf{st}\\
  \mc{D} \semi m_\mf{old} \vdash \mf{check}(m_\mf{new}, m_\mf{old}[p].child) 
  = m
} {
  \mc{D} \semi m_\mf{old} \vdash \mf{check}(m_\mf{new}, p) 
  = m
}
\and
\inferrule*[right=\hl{Check-PathDiff}] { 
  m_\mf{old}[p].\mf{st} \neq m_\mf{new}[p].\mf{st}
  \\
  m'[p].\mf{spec} = \pair{\mc{C}}{v}
  \and
  \mc{D}[\mc{C}] = x.\hat{e}
  \\
  m_\mf{new} \semi \hat{e}[v/x] \multistep{\mf{Succ}}{p} \eret{v'} \semi m
  \\
  \mc{D} \semi m \vdash \mf{init}(v') = \pair{t}{m'}
} {
  \mc{D} \semi m_\mf{old} \vdash \mf{check}(m_\mf{new}, p)
   = \pair{p}{m' \mid m[p].\mf{child}=t}
}
\end{mathpar}

\subsubsection{Handler Search}

Judgment: $m \vdash \mf{handlers}(t, id) = \num{v}$

\begin{mathpar}
\inferrule*[right=Handlers-Lit] { } {
  m \vdash \mf{handlers}(\mf{lit}\ v, id) = \{\}
}
\and
\inferrule*[right=Handlers-Comp] {
  m \vdash \mf{handlers}(m[p].\mf{child}, id) = \num{v}
} {
  m \vdash \mf{handlers}(p, id) = \num{v}
}
\and
\inferrule*[right=Handlers-Tgt] { } {
  m \vdash \mf{handlers}(\mf{node}(id, v, \num{v_i}, \num{v_j}), id)
  = \{v_h \mid \mf{onClick}\ v_h \in \num{v_i}\}
}
\and
\inferrule*[right=Handlers-Node] {
  id \neq id'
  \and
  (\forall j)\ 
  m \vdash \mf{handlers}(v_j, id) = h_j
} {
  m \vdash \mf{handlers}(\mf{node}(id', v, \num{v_i}, \num{v_j}), id)
  = \bigcup_{\forall j} h_j
}
\end{mathpar}

\subsubsection{Event Loop}

Judgment: $\four{\mc{D}}{m}{t}{\mu} \hookrightarrow \four{\mc{D}'}{m'}{t}{\mu'}$

\begin{mathpar}
\inferrule*[right=\hl{Loop-Start}] {
  \cdot \semi P \Mapsto^* \mf{run}(\eret{v}) \semi \mc{D}
  \\
  \mc{D} \semi \cdot \vdash \mf{init}(v) = \pair{t}{m'}
} {
  \four{\cdot}{\cdot}{\cdot}{\mf{start}(P)} 
  \hookrightarrow
  \four{\mc{D}}{m'}{t}{\mf{idle}}
}
\and
\inferrule*[right=\hl{Loop-Event}] { } {
  \four{\mc{D}}{m_0}{t}{\mf{idle}} \hookrightarrow \four{\mc{D}}{m}{t}{\mf{handle}(id)}
}
\and
\inferrule*[right=\hl{Loop-Handle}] {
  m \vdash \mf{handlers}(t, id) = \num{v_i}
  \\
  (0 \leq i < n)\quad
  m_i \semi v_i\ () \mapsto^* \eret{()} \semi m_{i+1}
} {
  \four{\mc{D}}{m_0}{t}{\mf{handle}(id)}
  \hookrightarrow
  \four{\mc{D}}{m}{t}{\mf{rerender}(m_n)}
}
\and
\inferrule*[right=\hl{Loop-Rerender}] {
  D \semi m_\mf{old} \vdash \mf{check}(m_\mf{new}, t) = m
} {
  \four{\mc{D}}{m_\mf{old}}{t}{\mf{rerender}(m_\mf{new})}
  \hookrightarrow
  \four{\mc{D}}{m}{t}{\mf{idle}}
}
\end{mathpar}

\subsection{Type Soundness}

Throughout, \emph{postulates} refer to (as-of-yet) unproved assumptions.

\subsubsection{General Lemmas}

We postulate weakening on $\Gamma$ for all typing judgments involving $\Gamma$.
For instance, the statement for weaking $\Gamma$ on values is:
\begin{postulate}[Weakening on $\Gamma$, values]
If $\Gamma \semi \Pi \vdval v: \tau$ and $\Gamma \po \Gamma'$,
then $\Gamma' \semi \Pi \vdval v: \tau$.
\end{postulate}

Similarly, we require weakening on $\Pi$ for all typing judgments involving $\Pi$:
\begin{postulate}[Weakening on $\Pi$, values]
If $\Gamma \semi \Pi \vdval v: \tau$ and $\Pi \po \Pi'$,
then $\Gamma \semi \Pi' \vdval v: \tau$.
\end{postulate}

We elide statements of all other weakening lemmas.

\begin{postulate}[Canonical Forms]
If $\cdot \semi \Pi \vdval v: \tau$, then:
\begin{itemize}
\item if $\tau=\one$, then $v=()$.
\item if $\tau=\mf{Bool}$, then either $v=\mf{true}$ or $v=\mf{false}$.
\item if $\tau=\mf{String}$, then $v=s$.
\item if $\tau=\tau_1 \xrightarrow{E} \tau_2$, then $v=\lambda x: \tau_1.e$.
\item if $\tau=\mf{View}$, then either $v=\mf{text}\ v'$,
    $v=\mf{tag}(v' \num{v_i}, \num{v_j})$, or $v=\pair{\mc{C}}{v'}$.
\item if $\tau=\mf{Attr}$, then either $v=\mf{attr}(v_1, v_2)$ or $v=\mf{onClick}\ v'$.
\item if $\tau=\mf{Component}\ \tau'$, then $v=\mc{C}$.
\item if $\tau=\mf{Setter}\ \tau'$, then $v=\vset{\ell}{p}$.
\end{itemize}
\end{postulate}

We postulate substitution lemmas on all typing judgments involving $\Gamma$.
For instance:
\begin{postulate}[Substitution lemma, expressions]
If $\Gamma[x \mapsto \tau_v] \semi \Pi \vdexp e: \tau \mid E$
and $\Gamma \semi \Pi \vdval v: \tau_v$,
then $\Gamma \semi \Pi \vdexp e[v/x]: \tau \mid E$.
\end{postulate}

We elide the statement of analogous lemmas on values,
top-level expressions, and programs.

\begin{postulate}[Label Preservation over Substitution]
For any $\hat{e}$, $v$, and $x$, we have $\mc{L}(\hat{e}) = \mc{L}(\hat{e}[v/x])$.
\end{postulate}

\begin{lemma}[Pure Progress]
If $m \semi e \mapsto e' \semi m'$ 
and $\Gamma \semi \Pi \vdexp e: \tau \mid \epsilon$,
then $m' = m$.
\end{lemma}
\begin{proof}
By induction over the derivation of the small-step judgment.

\textbf{Case} \textsc{E-Let}:
\begin{itemize}
\item We have
$m \semi \elet{x}{e_1}{e_2} \mapsto \elet{x}{e_1'}{e_2}$
and $m \semi e_1 \mapsto e_1' \semi e_2$.
\item By the typing premise, we get $\Gamma \semi \Pi \vdexp e_1: \tau_1 \mid \epsilon$.
\item By inductive hypothesis, we get $m' = m$ and we are done.
\end{itemize}

\textbf{Case} \textsc{E-AppSet}: Impossible,
since by \textsc{TExp-AppSet}, 
$\Gamma \semi \Pi \vdexp \vset{\ell}{p}\ v: \one \mid \mf{Mut}$,
i.e. the expression can never have an empty effect.\\
\hl{(How to express this idea?)}

\textbf{All other cases}: 
The appropriate rule immediately gives $m'=m$ and we are done.
\end{proof}

\subsubsection{Expression Soundness}

\begin{lemma}[Progress of expressions]
If $\cdot \semi \Pi \vdexp e: \tau \mid E$, then either:
\begin{enumerate}[(i)]
\item $e = \eret{v}$, or
\item for all $m$ such that $\cdot \semi \Pi \vdmwk m$, 
there exist some $e'$ and $m'$ such that 
$m \semi e \mapsto e' \semi m'$.
\end{enumerate}
\end{lemma}

\begin{proof}
By induction on the derivation of the typing judgment.

\textbf{Case} \textsc{TExp-Ret}: \begin{itemize}
\item We have 
$\cdot \semi \Pi \vdexp \eret{v}: \tau \mid \epsilon$.
\item The theorem holds trivially by case \emph{(i)}.
\end{itemize}

\textbf{Case} \textsc{TExp-Let}: \begin{itemize}
\item We have 
$\cdot \semi \Pi \vdexp \elet{x}{e_1}{e_2}: \tau_2 \mid E$.
\item We also get $\cdot \semi \Pi \vdexp e_1: \tau_1 \mid E$.
\item By inductive hypothesis on this statement, we have two subcases on $e_1$:
either (a) $e_1 = \eret{v}$,
or (b) for any $m$ with $\Pi \vDash^\mf{wk}_\Delta m$, we have
$m \semi e_1 \mapsto e_1' \semi m'$.
\item Subcase (a): \begin{itemize}
\item We apply \textsc{E-Ret}
to get $m \semi \elet{x}{\eret{v}}{e_2} \mapsto e_2[v/x] \semi m$
for any $m$.
\end{itemize}
\item Subcase (b): 
\begin{itemize}
  \item We apply \textsc{E-Let} to get 
  $m \semi \elet{x}{e_1}{e_2} \mapsto \elet{x}{e_1'}{e_2} \semi m'$.
\end{itemize}
\item Thus, in any subcase, $\elet{x}{e_1}{e_2}$ steps (with well-typed $m$),
and we satisfy conclusion \emph{(ii)}.
\end{itemize}


\textbf{Case} \textsc{TExp-AppFun}: \begin{itemize}
\item We have $\cdot \semi \Pi \vdexp v_1\ v_2: \tau_2 \mid E$.
\item It follows that $\cdot \semi \Pi \vdval v_1 : \tau_1 \xrightarrow{E} \tau_2$.
\item By canonical forms, $v_1=\lambda x.e$.
\item We apply \textsc{E-AppFun} to get (for any $m$) that
$m \semi (\lambda x.e) v_2 \mapsto e[v_2/x] \semi m$,
satisfying conclusion \emph{(ii)}.
\end{itemize}

\textbf{Case} \textsc{TExp-AppComp}: \begin{itemize}
\item We have $\cdot \semi \Pi \vdexp v_1\ v_2: \mf{View} \mid \epsilon$.
\item Also, $\cdot \semi \Pi \vdval v_1: \mf{Component}\ \tau$.
\item By canonical forms, $v_1 = \mc{C}$.
\item We apply \textsc{E-AppComp} to get (for any $m$) that
$m \semi \mc{C}\ v_2 \mapsto \pair{\mc{C}}{v_2} \semi m$,
satisfying conclusion \emph{(ii)}.
\end{itemize}

\textbf{Case} \textsc{TExp-AppSet}: \begin{itemize}
\item We have $\cdot \semi \Pi \vdexp v_1\ v_2: \one \mid \mf{Mut}$.
\item It follows that $\cdot \semi \Pi \vdval v_1 : \mf{Setter}\ \tau$.
\item By canonical forms, $v_1 = \vset{\ell}{p}$.
\item We apply \textsc{E-AppSet} to get that
$m \semi \vset{\ell}{p}\ v_2 \mapsto \eret{()} \semi m'$,
where $m' = m \mid m[p].\mf{st}[\ell] = v_2$,
and we thus satisfy conclusion \emph{(ii)}.
\end{itemize}

\textbf{Case} \textsc{TExp-If}: \begin{itemize}
\item We have $\cdot \semi \Pi \vdexp \eif{v}{e_1}{e_2} : \tau \mid E$.
\item It follows that $\cdot \semi \Pi \vdexp v: \mf{Bool}$.
\item By canonical forms, either $v=\mf{true}$ or $v=\mf{false}$, giving two subcases.
\begin{itemize}
\item If $v=\mf{true}$, then \textsc{E-IfTrue} says 
$m \semi \eif{v}{e_1}{e_2} \mapsto e_1 \semi m$.
\item If $v=\mf{false}$, ththen \textsc{E-IfFalse} says
$m \semi \eif{v}{e_1}{e_2} \mapsto e_2 \semi m$.
\end{itemize}
\item In either subcase, we satisfy conclusion \emph{(ii)}.
\end{itemize}

\textbf{Case} \textsc{TExp-Sub}: \begin{itemize}
\item We have $\cdot \semi \Pi \vdexp e : \tau \mid \mf{Mut}$.
\item It follows from the derivation that
$\cdot \semi \Pi \vdexp e : \tau \mid \epsilon$.
\item We apply the inductive hypothesis with the more conservative typing premise
and we are done.
\end{itemize}

\end{proof}

\begin{lemma}[Preservation of expressions]
If
\begin{enumerate}[(i)]
\item $\Gamma \semi \Pi \vdexp e: \tau \mid E$,
\item $\Gamma \semi \Pi \vdmwk m$ for arbitrary $m$, and
\item $m \semi e \mapsto e' \semi m'$ for some $e'$ and $m'$,
\end{enumerate}
then
\begin{enumerate}[(i)]
\item $\Gamma \semi \Pi \vdexp e': \tau \mid E$ and
\item $\Gamma \semi \Pi \vdmwk m'$.
\end{enumerate}
\end{lemma}

\begin{proof}
By induction on derivation of \emph{(iii)}.

\textbf{Case} \textsc{E-Let}: \begin{itemize}
\item We have
$m \semi \elet{x}{e_1}{e_2} \mapsto \elet{x}{e_1'}{e_2} \semi m'$.
\item Also, (a) $m \semi e_1 \mapsto e_1' \semi m'$.
\item By \emph{(i)}, $\Gamma \semi \Pi \vdexp \elet{x}{e_1}{e_2} : \tau \mid E$;
it follows by \textsc{TExp-Let} that \begin{enumerate}[(a)]
  \setcounter{enumi}{1}
  \item $\Gamma \semi \Pi \vdexp e_1: \tau_1 \mid E$, and
  \item (c) $\Gamma[x \mapsto \tau_1] \semi \Pi \vdexp e_1: \tau \mid E$.
\end{enumerate}
\item Applying the inductive hypothesis to (a) and (c) gives
$\Gamma \semi \Pi \vDash^\mf{wk}_\Delta m'$, i.e. conlcusion \emph{(i)}.
\item We also get via inductive hypothesis that 
$\Gamma \semi \Pi \vdexp e_1': \tau_1 \mid E_1$.
\item Reapplying \textsc{TExp-Let} alongside (b) gives 
$\Gamma \semi \Pi \vdexp \elet{x}{e_1'}{e_2}: \tau \mid E$,
i.e. conclusion \emph{(ii)}, and we are done.
\end{itemize}

\textbf{Case} \textsc{E-Ret}: \begin{itemize}
\item We have $m \semi \elet{x}{\eret{v}}{e} \mapsto e[v/x] \semi m$.
\item \emph{(i)} gives that 
$\Gamma \semi \Pi \vdexp \elet{x}{\eret{v}}{e} : \tau \mid E$.
\item By \textsc{TExp-Ret}, $\Gamma \semi \Pi \vdval v: \tau_1$ 
and $\Gamma[x \mapsto \tau_1] \semi \Pi \vdexp e : \tau \mid E$.
\item We apply the substitution lemma to get 
$\Gamma \semi \Pi \vdexp e[v/x] : \tau \mid E$ and we are done.
\end{itemize}

\textbf{Case} \textsc{E-AppFun}: \begin{itemize}
\item We have $m \semi (\lambda x.e) v \mapsto e[v/x] \semi m$.
\item By \emph{(i)} and \textsc{TExp-AppFun} we get $\Gamma \semi \Pi \vdexp v: \tau_1$
and $\Gamma[x \mapsto \tau_1] \semi \Pi \vdexp e : \tau \mid E$.
\item By substitution lemma, we conclude that 
$\Gamma \semi \Pi \vdexp e[v/x] : \tau \mid E$ and we are done.
\end{itemize}

\textbf{Case} \textsc{E-AppComp}: \begin{itemize}
\item We have $m \semi \mc{C}\ v \mapsto \eret{\pair{\mc{C}}{v}} \semi m$.
\item By \emph{(i)} and \textsc{TVal-Spec},
$\Gamma \semi \Pi \vdexp \mc{C}\ v: \mf{View} \mid \epsilon$.
\item It follows that $\Gamma \semi \Pi \vdval \mc{C}: \mf{Component}\ \tau$
and $\Gamma \semi \Pi \vdval v: \tau$.
\item We reapply \textsc{TVal-Spec} to get 
$\Gamma \semi \Pi \vdval \pair{\mc{C}}{v}: \mf{View}$.
\item Then, by \textsc{TExp-Ret}, 
$\Gamma \semi \Pi \vdexp \eret{\pair{\mc{C}}{v}}: \mf{View} \mid \epsilon$
and we are done.
\end{itemize}

\textbf{Case} \textsc{E-AppSet}: \begin{itemize}
\item We have $m \semi \vset{\ell}{p}\ v \mapsto \eret{()} \semi m'$,
where $m' = m \mid m[p].\mf{st}[\ell] = v$.
\item By \emph{(i)} and \textsc{TVal-AppSet},
$\Gamma \semi \Pi \vdexp \vset{\ell}{p}\ v: \one \mid \mf{Mut}$.
\item It follows that 
\begin{enumerate}[(a)]
  \item $\Gamma \semi \Pi \vdval \vset{\ell}{p}: \mf{Setter}\ \tau$ and
  \item $\Gamma \semi \Pi \vdval v: \tau$.
\end{enumerate}
\item Conclusion \emph{(i)} follows from the derivation:
\begin{mathpar}
\inferrule*[right=TExp-Sub] {
  \inferrule*[right=TExp-Ret] {
    \inferrule*[right=TVal-Unit] { } {
      \Gamma \semi \Pi \vdval (): \one
    }
  } {
    \Gamma \semi \Pi \vdexp \eret{()}: \one \mid \epsilon
  }
} {
  \Gamma \semi \Pi \vdexp \eret{()}: \one \mid \mf{Mut}
}
\end{mathpar}
\item To show conclusion \emph{(ii)}, 
since $m'$ differs from $m$ only at $m[p].\mf{st}[\ell]$,
it suffices to show that $\Gamma \vdval m'[p].\mf{st}[\ell] : \Delta[\Pi[p]][\ell]$.
\item By definition of $m'$, we want to show 
$\Gamma \vdval v : \Delta[\Pi[p]][\ell]$.
\item It follows from (a) that $\Delta[\Pi[p]][\ell] = \tau_1$.
\item Thus, the desired conclusion is given by (b), and we are done.
\end{itemize}

\textbf{Case} \textsc{E-IfTrue}: \begin{itemize}
\item We have $m \semi \eif{\mf{true}}{e_1}{e_2} \mapsto e_1 \semi m$.
\item By \emph{(i)}, $\Gamma \semi \Pi \vdexp \eif{v}{e_1}{e_2}: \tau \mid E$.
\item By \textsc{TExp-If}, $\Gamma \semi \Pi \vdexp e_1: \tau \mid E$ and we are done.
\end{itemize}

\textbf{Case} \textsc{E-IfFalse}: Analogous to \textsc{E-IfTrue}; elided.
\end{proof}

\subsubsection{Top-Level Expressions, Initial Phase}

\begin{lemma}[Progress of top-level expressions, initial phase]
If $\cdot \semi \Pi \semi \mc{C} \vdtop \hat{e}$, then either:
\begin{enumerate}[(i)]
\item $\hat{e} = \eret{v}$, or
\item for all $m$ such that $\cdot \semi \Pi \vdmwk m$
and arbitrary $p \in \Pi$,
there exist some $\hat{e}'$ and $m'$ such that
$m \semi \hat{e} \topstep{\mf{Init}}{p} \hat{e}' \semi m'$.
\end{enumerate}
\end{lemma}

\begin{proof}
By induction on the derivation of the typing judgment.

\textbf{Case} \textsc{THat-Lift}: \begin{itemize}
\item We have $\cdot \semi \Pi \vdexp e: \mf{View} \mid \epsilon$.
\item By progress of expressions, we have two subcases.
\item Subcase (a): $e=\eret{v}$. 
\begin{itemize}
  \item We trivially satisfy conclusion \emph{(i)} and we are done.
\end{itemize}
\item Subcase (b): for all $m$ with $\cdot \semi \Pi \vDash^\mf{wk}_\Delta m$, 
there exist some $e'$ and $m'$ such that
$m \semi e \mapsto e' \semi m'$.
\begin{itemize}
  \item By definition of $\vdmwk$, $m=\cdot$.
  \item By pure progress, $m' = \cdot$.
  \item Apply \textsc{E-Lift} to get
$m \semi \hat{e} \topstep{\mf{Init}}{p} \hat{e}' \semi m$,
satisfying conclusion \emph{(ii)}.
\end{itemize}
\end{itemize}

\textbf{Case} \textsc{THat-Let}: \begin{itemize}
\item We have $\cdot \semi \Pi \vdexp e: \tau_1 \mid \epsilon$.
\item Again by progress of expressions, we have two subcases.
\item Subcase (a): $e = \eret{v}$. \begin{itemize}
  \item Apply \textsc{E-RetTop} to get
  $m \semi \elet{x}{\eret{v}}{\hat{e}} \topstep{\mf{Init}}{p} \hat{e}[v/x] \semi m$.
\end{itemize}
\item Subcase (b): for all $m$ with $\cdot \semi \Pi \vDash^\mf{wk}_\Delta m$, 
there exist some $e'$ and $m'$ such that
$m \semi e \mapsto e' \semi m'$.\begin{itemize}
  \item By definition of $\vdmwk$, $m=\cdot$.
  \item By pure progress, $m' = \cdot$.
  \item Apply \textsc{E-LetTop} to get
  $m \semi \elet{x}{e}{\hat{e}} \topstep{\mf{Init}}{p} \elet{x}{e'}{\hat{e}} \semi m$.
\end{itemize}
\item In any subcase, we satisfy conclusion \emph{(ii)} and we are done.
\end{itemize}
\textbf{Case} \textsc{THat-State}:
We simply apply \textsc{E-StInit} and we are done.
\end{proof}

\begin{lemma}[Preservation of top-level expression, initial phase]
If
\begin{enumerate}[(i)]
\item $\Gamma \semi \Pi \semi \Pi[p] \vdtop \hat{e}$,
\item $\Gamma \semi \Pi \vDash^\mf{wk}_\Delta m$,
\item $m \semi \hat{e} \topstep{\mf{Init}}{p} \hat{e}' \semi m'$,
\end{enumerate}
then
\begin{enumerate}[(i)]
\item $\Gamma \semi \Pi \semi \Pi[p] \vdtop \hat{e}'$ and
\item $\Gamma \semi \Pi \vDash^\mf{wk}_\Delta m'$.
\end{enumerate}
\end{lemma}

\begin{proof}
By induction on derivation of \emph{(iii)}.

\textbf{Case} \textsc{E-Lift}: \begin{itemize}
\item We have $\cdot \semi e \mapsto e' \semi \cdot$.
\item By premise \emph{(i)} and \textsc{THat-Lift},
$\Gamma \semi \Pi \vdexp e: \mf{View} \mid \epsilon$.
\item We apply preservation of expressions to get
(a) $\Gamma \semi \Pi \vdexp e': \mf{View} \mid \epsilon$.
\item We reapply \textsc{THat-Lift} to (a) to satisfy conclusion \emph{(i)} and we are done.
\end{itemize}

\textbf{Case} \textsc{E-LetTop}: \begin{itemize}
\item We have
$m \semi \elet{x}{e}{\hat{e}} \topstep{\mf{Init}}{p} \elet{x}{e'}{\hat{e}} \semi m'$.
\item Also, (a) $\cdot \semi e \mapsto e' \semi \cdot$.
\item By premise \emph{(i)} and \textsc{THat-Let},
(b) $\Gamma \semi \Pi \semi \vdexp e: \tau \mid \epsilon$
and (c) $\Gamma[x \mapsto \tau] \semi \Pi \semi \Pi[p] \vdtop \hat{e}$.
\item Preservation of expressions on (a) and (b) gives that 
(d) $\Gamma \semi \Pi \vdexp e': \tau \mid \epsilon$.
\item We reapply \textsc{THat-Let} to (c) and (d) to satisfy conclusion \emph{(i)}
and we are done.
\end{itemize}

\textbf{Case} \textsc{E-RetTop}:\begin{itemize}
\item We have
$m \semi \elet{x}{\eret{v}}{\hat{e}} \topstep{\mf{Init}}{p} \hat{e}[v/x] \semi m$.
\item By premise \emph{(i)} and \textsc{THat-Let},
we have $\Gamma \semi \Pi \vdval v: \tau_1$
and $\Gamma[x \mapsto \tau_1] \semi \Pi \semi \Pi[p] \vdtop \hat{e}$.
\item By substitution lemma,
we get that $\Gamma \semi \Pi \semi \Pi[p] \vdtop \hat{e}[v/x]$
and we are done.
\end{itemize}

\textbf{Case} \textsc{E-StInit}: \begin{itemize}
\item We have
$m \semi \estate{x}{\ell}{v}{\hat{e}} \topstep{\mf{Init}}{p}
\hat{e}[v/x, \vset{\ell}{p}/x_\mf{set}] \semi m'$
where $m' = m \mid m[p].\mf{st}[\ell] = v$.
\item By premise \emph{(i)} and \textsc{THat-State}, we get
\begin{enumerate}[(a)]
\item $\Delta[\Pi[p]][\ell] = \tau_1$,
\item $\Gamma \semi \Pi \vdval v: \tau_1$, and
\item $\Gamma[x \mapsto \tau_1, x_\mf{set} \mapsto \mf{Setter}\ \tau_1] 
\semi \Pi \semi \Pi[p] \vdtop \hat{e}$.
\end{enumerate}
\item We apply \textsc{TVal-Setter} to (a) to get (d)
$\Gamma \semi \Pi \vdval \vset{\ell}{p}: \mf{Setter}\ \tau_1$.
\item Two applications of the substitution lemma to (c), using (b) and (d),
yield $\Gamma \semi \Pi \semi \Pi[p] \vdtop \hat{e}[v/x, \vset{\ell}{p}/x_\mf{set}]$,
satisfying conclusion \emph{(i)}.
\item To show conclusion \emph{(ii)}, it suffices to show that
$\Gamma \semi \Pi \vdval m'[p].\mf{st}[\ell] : \Delta[\Pi[p]][\ell]$.
\item This is given by (a), (b), and definition of $m'$, and we are done.
\end{itemize}

\textbf{Case} \textsc{E-StSucc}: Impossible,
since the judgment requires that $\phi=\mf{Succ}$.

\end{proof}

\begin{theorem}[Soundness of top-level expressions, initial phase.]
If 
\begin{enumerate}[(i)]
\item $\cdot \semi \Pi \vdtop \hat{e}$,
\item $\cdot \semi \Pi \vDash^\mf{wk}_\Delta m$, and
\item $m \semi \hat{e} \Topstep{\mf{Init}}{p} v \semi m'$,
\end{enumerate}
then 
\begin{enumerate}[(i)]
\item $\cdot \semi \Pi \vdval v: \mf{View}$ and
\item $\cdot \semi \Pi \vDash^\mf{wk}_\Delta m'$.
\end{enumerate}
\end{theorem}

\begin{proof}
Follows from progress \& preservation.
\end{proof}

\subsubsection{Programs}

\begin{lemma}[Progress of programs]
If $\cdot \vdprog P$, then either:
\begin{enumerate}[(i)]
\item $P = \eret{v}$, or
\item for all $\mc{D}$ with $\Delta \vDash \mc{D}$,
there exist some $P'$ and $\mc{D}'$ such that
$\mc{D} \semi P \pstep P' \semi \mc{D}'$.
\end{enumerate}
\end{lemma}

\begin{proof}
By induction over the derivation of the typing judgment.

\textbf{Case} \textsc{TProg-Lift}: \begin{itemize}
\item We have $\cdot \semi \cdot \vdexp e: \mf{View} \mid \epsilon$.
\item By progress of expressions, we have two subcases.
\item Subcase (a): $e=\eret{v}$.
\begin{itemize}
  \item Conclusion \emph{(i)} is trivially satisfied.
\end{itemize}
\item Subcase (b): for all $m$ with $\cdot \semi \cdot \vdmwk m$,
there exist some $e'$ and $m'$ with $m \semi e \mapsto e' \semi m$.
\begin{itemize}
  \item By definition of $\vdmwk$, we must have $m=\cdot$.
  \item By pure progress, $m' = \cdot$.
  \item We can therefore apply \textsc{P-Lift} to get 
$\mc{D} \semi e \pstep e' \semi \mc{D}$,
thus satisfying conclusion \emph{(ii)}.
\end{itemize}
\end{itemize}

\textbf{Case} \textsc{TProg-Let}: \begin{itemize}
\item We have $\cdot \semi \cdot \vdexp e: \mf{\tau} \mid \epsilon$.
\item By progress of expressions, we again have two subcases.
\item Subcase (a): $e=\eret{v}$.
\begin{itemize}
  \item We apply \textsc{P-Ret} to get
  $\mc{D} \semi \elet{x}{\eret{v}}{P} \pstep P[v/x] \semi \mc{D}$
  and we are done.
\end{itemize}
\item Subcase (b): for all $m$ with $\cdot \semi \cdot \vdmwk m$,
there exist some $e'$ and $m'$ with $m \semi e \mapsto e' \semi m$.
\begin{itemize}
  \item By definition of $\vdmwk$, we must have $m=\cdot$.
  \item By pure progress, $m' = \cdot$.
  \item We can therefore apply \textsc{P-Let} to get 
$\mc{D} \semi \elet{x}{e}{P} \pstep \elet{x}{e'}{P} \semi \mc{D}$,
thus satisfying conclusion \emph{(ii)}.
\end{itemize}
\end{itemize}

\textbf{Case} \textsc{TProg-Comp}: We simply apply \textsc{P-Comp}
and we are done.
\end{proof}

\begin{lemma}[Preservation of programs.]
If
\begin{enumerate}[(i)]
\item $\Gamma \vdprog P$,
\item $\Delta \vDash \mc{D}$, and
\item $\mc{D} \semi P \pstep P' \semi \mc{D}'$,
\end{enumerate}
then
\begin{enumerate}[(i)]
\item $\Gamma \vdprog P'$ and
\item $\Delta \vDash \mc{D}'$.
\end{enumerate}
\end{lemma}

\begin{proof}
By induction over the derivation of the small-step judgment.

\textbf{Case} \textsc{P-Lift}: \begin{itemize}
\item We have $\cdot \semi e \mapsto e' \semi \cdot$.
\item By premise \emph{(i)} and \textsc{TProg-Lift},
we have $\Gamma \semi \cdot \vdexp e : \mf{View} \mid \epsilon$.
\item By preservation of expressions,
$\Gamma \semi \cdot \vdexp e': \mf{View} \mid \epsilon$.
\item Reapplication of \textsc{TProg-Lift} gives $\Gamma \vdprog e'$
and we are done.
\end{itemize}

\textbf{Case} \textsc{P-Let}: \begin{itemize}
\item We again have (a) $\cdot \semi e \mapsto e' \semi \cdot$.
\item By premise \emph{(i)} and \textsc{TProg-Let},
we have (b) $\Gamma \semi \cdot \vdexp e : \tau \mid \epsilon$
and (c) $\Gamma[x \mapsto \tau] \vdprog P$.
\item By preservation of expressions on (a) and (b), we get (d)
$\Gamma \semi \cdot \vdexp e': \tau \mid \epsilon$.
\item Reapplication of \textsc{TProg-Let} to (c) and (d) gives 
$\Gamma \vdprog \elet{x}{e'}{P}$ and we are done.
\end{itemize}

\textbf{Case} \textsc{P-Ret}: \begin{itemize}
\item We have $\mc{D} \semi \elet{x}{\eret{v}}{P} \pstep P[v/x] \semi \mc{D}$.
\item Premise \emph{(i)}
gives $\Gamma \semi \cdot \vdval v: \tau$
and $\Gamma, x: \tau \vdprog P$.
\item By substitution lemma,
$\Gamma \vdprog P[v/x]$ and we are done.
\end{itemize}

\textbf{Case} \textsc{P-Comp}: \begin{itemize}
\item We immediately get from premise \emph{(i)}
that $\Gamma \vdprog P$, satisfying conclusion \emph{(i)}.
\item We also need to show that $\Delta \vDash \mc{D}'$.
\item By premise \emph{(ii)},
it suffices to show that 
$[x \mapsto \Delta[\mc{C}]] \semi \cdot \semi \mc{C} \vdtop \hat{e}$.
\item This is also given by premise \emph{(i)}:
we get that $x: \tau \semi \cdot \semi \mc{C} \vdtop \hat{e}$
and $\Delta[\mc{C}] = \tau$.
\end{itemize}
\end{proof}

\begin{theorem}[Soundness of programs]
If $\cdot \vdprog P$ and $\cdot \semi P \Pstep v \semi \mc{D}$,
then $\cdot \semi \cdot \vdval v: \mf{View}$ and $\Delta \vDash \mc{D}$.
\end{theorem}

\begin{proof}
Follows from progress \& preservation of programs.
\end{proof}

\subsubsection{Initialization}

We first prove a number of lemmas which are necessary in our fundamental property.

\begin{lemma}[Path Preservation]
If $m \semi \hat{e} \topstep{\phi}{p} \hat{e}' \semi m'$
and $p \in \dom(m)$,
then $\dom(m) = \dom(m')$.
\end{lemma}

\begin{proof}
By induction on the derivation of the small-step judgment.

\textbf{Case} \textsc{E-StInit}: \begin{itemize}
\item We have
$m \semi \estate{x}{\ell}{v}{\hat{e}} \topstep{\mf{Init}}{p}
\hat{e}[v/x, \vset{\ell}{p}/x_\mf{set}] \semi m'$
where $m' = m \mid m[p].\mf{st}[\ell] = v$.
\item Since we assumed $p \in \dom(m)$, the addition of $p$
to $m'$ does not change its domain, and therefore we have $\dom(m)=\dom(m')$.
\end{itemize}

\textbf{All other cases} do not alter $m$, and the lemma holds vacuously.

\end{proof}

\begin{corollary}
If $m \semi \hat{e} \Topstep{\phi}{p} v \semi m'$
and $p \in \dom(m)$,
then $\dom(m) = \dom(m')$.
\end{corollary}

\begin{proof}
Immediate from path preservation and definition of $\Topstep{\phi}{p}$.
\end{proof}

\begin{lemma}[Memory Freshness]
If $\mc{D} \vdash v \init \pair{t}{m}$,
then for all $p \in \dom(m)$, we have $p\ \mf{fresh}$.
\end{lemma}

\begin{proof}
By induction over the derivation of the big-step judgment.

\textbf{Case} \textsc{Init-Lit}: $m=\cdot$, so the conclusion holds vacuously.

\textbf{Case} \textsc{Init-Node}: \begin{itemize}
\item We have
$\mc{D} \vdash \mf{tag}(v, \num{v_i}, \num{v_j}) 
\init
\pair{\mf{node}(\mf{id}, v, \num{v_i}, \num{t_j})}{\bigcup_j m_j}$.
\item For all $j$, we also have $\mc{D} \vdash v_j \init \pair{t_j}{m_j}$.
\item We apply the inductive hypothesis to each, getting that
all $m_j$ only contain fresh $p$.
\item Therefore, their union $\bigcup_j m_j$ also only contains fresh $p$,
and we are done.
\end{itemize}

\textbf{Case} \textsc{Init-Comp}: \begin{itemize}
\item We start with $\mc{D} \vdash \pair{\mc{C}}{v} 
\init \pair{p}{m' \cup (m \mid m[p].\mf{child} = t)}$.
\item Let $\mc{D}[\mc{C}] = x.\hat{e}$ and
$m_0 = \{\mf{spec}: \pair{\mc{C}}{v_0}, \mf{st}: \emptyset, \mf{child}: \emptyset\}$.
\item We have the following premises:
\begin{enumerate}[(a)]
  \item $p$ is fresh,
  \item $m_0 \semi \hat{e}[v_0/x] \Topstep{\mf{Init}}{p} v \semi m$, and
  \item $\mc{D} \vdash v \init \pair{t}{m'}$.
\end{enumerate}
\item By (a), clearly $m_0$ has all $p$ fresh.
\item By the path preservation corollary on (b), $m$ also has all $p$ fresh.
\item By inductive hypothesis on $c$, $m'$ also has all $p$ fresh.
\item Therefore, $m' \cup (m \mid m[p].\mf{child} = t)$
has all $p$ fresh and we are done.
\end{itemize}
\end{proof}

\begin{lemma}[Label Initialization]
If $m \semi \hat{e} \topstep{\mf{Init}}{p} \hat{e}' \semi m'$,
then $\mc{L}(\hat{e}) \setminus \mc{L}(\hat{e}') \subseteq \dom(m'[p].\mf{st})$.
\end{lemma}

\begin{proof}
By case-analysis of the small-step judgment. Note that if 
$\mc{L}(\hat{e}) \setminus \mc{L}(\hat{e}') = \emptyset$,
the conclusion holds vacuously.

\textbf{Case} \textsc{E-Lift}: \begin{itemize}
\item We have $\cdot \semi e \mapsto e' \semi \cdot$.
\item By definition, $\mc{L}(e)=\mc{L}(e')=\emptyset$.
\item Clearly then $\mc{L}(e) \setminus \mc{L}(e')=\emptyset$ and we are done.
\end{itemize}

\textbf{Case} \textsc{E-LetTop}: \begin{itemize}
\item We have $m \semi \elet{x}{e}{\hat{e}} \topstep{\mf{Init}}{p} \elet{x}{e'}{\hat{e}} \semi m$,
with $\cdot \semi e \mapsto e' \semi \cdot$.
\item By definition, $\mc{L}(\elet{x}{e}{\hat{e}})=\mc{L}(\elet{x}{e'}{\hat{e}})=\mc{L}(\hat{e})$.
\item We get $\mc{L}(\elet{x}{e}{\hat{e}}) \setminus \mc{L}(\elet{x}{e'}{\hat{e}})=\emptyset$ and we are done.
\end{itemize}

\textbf{Case} \textsc{E-RetTop}: \begin{itemize}
\item We have $m \semi \elet{x}{\eret{v}}{\hat{e}} 
\topstep{\mf{Init}}{p} \hat{e}[v/x] \semi m$.
\item By definition, $\mc{L}(\elet{x}{\eret{v}}{\hat{e}})=\mc{L}(\hat{e})$.
\item By label preservation over substitution, 
$\mc{L}(\hat{e}[v/x])=\mc{L}(\hat{e})$.
\item We get $\mc{L}(\elet{x}{\eret{v}}{\hat{e}}) 
\setminus \mc{L}(\hat{e}[v/x])=\emptyset$ and we are done.
\end{itemize}

\textbf{Case} \textsc{E-StInit}: \begin{itemize}
\item We have $m \semi \estate{x}{\ell}{v}{\hat{e}} 
\topstep{\mf{Init}}{p} \hat{e}[v/x, \vset{\ell}{p}/x_\mf{set}]
\semi m'$, where $m' = m \mid m[p].\mf{st}[\ell] = v$.
\item By definition, $\mc{L}(\estate{x}{\ell}{v}{\hat{e}}) 
= \{\ell\} \cup \mc{L}(\hat{e})$.
\item By label preservation over substitution,
$\mc{L}(\hat{e}[v/x, \vset{\ell}{p}/x_\mf{set}]) = \mc{L}(\hat{e})$.
\item We subcase on whether $\ell \in \mc{L}(\hat{e})$.
\item Subcase 1: $\ell \in \mc{L}(\hat{e})$.
\begin{itemize}
  \item It follows that $\mc{L}(\estate{x}{\ell}{v}{\hat{e}}) = \mc{L}(\hat{e})$.
  \item Thus, $\mc{L}(\estate{x}{\ell}{v}{\hat{e}}) \setminus
\mc{L}(\hat{e}[v/x, \vset{\ell}{p}/x_\mf{set}]) = \emptyset$ and we are done.
\end{itemize}
\item Subcase 2: $\ell \notin \mc{L}(\hat{e})$.
\begin{itemize}
  \item Then $\mc{L}(\estate{x}{\ell}{v}{\hat{e}}) \setminus \mc{L}(\hat{e}) = \{\ell\}$.
  \item Thus, we need to show $\ell \in m'[p].\mf{st}$.
  \item This is given by definition of $m'$, and we are done.
\end{itemize}
\end{itemize}

\textbf{Case} \textsc{E-StSucc}: Impossible, 
since this case requires $\phi=\mf{Succ}$,
which contradicts our assumption that $\phi=\mf{Init}$.

\end{proof}

\begin{corollary}
If $m \semi \hat{e} \Topstep{\mf{Init}}{p} v \semi m'$,
then $\mc{L}(\hat{e}) \subseteq \dom(m'[p].\mf{st})$.
\end{corollary}

\begin{proof}
Follows from label initialization and the fact that 
$\mc{L}(\eret{v})=\emptyset$, which means that 
$\mc{L}(\hat{e}) \setminus \mc{L}(\eret{v}) = \mc{L}(\hat{e})$.
\end{proof}

\begin{lemma}[Strong Disjoint Memory]
If \begin{enumerate}[(i)]
\item $\Gamma \semi \Pi \semi \mc{D} \vdmstr m$,
\item $\Gamma \semi \Pi \semi \mc{D} \vdmstr m'$, and
\item $\dom(m) \cap \dom(m') = \emptyset$,
\end{enumerate}
then $\Gamma \semi \Pi \semi \mc{D} \vdmstr m \cup m'$.
\end{lemma}

\begin{proof}
Follows easily from definition of $\vdmstr$.
\end{proof}

\begin{theorem}[Fundamental Property of Initialization]
If \begin{enumerate}[(i)]
\item $\mc{D} \vdash v \init \pair{t}{m}$,
\item $\Delta \vDash \mc{D}$, and
\item $\cdot \semi \Pi \vdval v: \mf{View}$,
\end{enumerate}
then there exists some $\Pi'$ such that: \begin{enumerate}[(i)]
\item $\Pi \sqsubseteq \Pi'$,
\item $\Pi' \vdtree t$, and
\item $\cdot \semi \Pi' \semi \mc{D} \vdmstr m$.
\end{enumerate}
\end{theorem}

\begin{proof}
By induction on derivation of \emph{(i)}.

\textbf{Case} \textsc{Init-Lit}: \begin{itemize}
\item We have $\mc{D} \vdash \mf{text}\ v \init \pair{\mf{lit}\ v}{\cdot}$.
\item Choose $\Pi'=\Pi$; conclusions \emph{(i)} and \emph{(iii)} are trivial.
\item By premise \emph{(iii)} and \textsc{TVal-Text},
we have $\cdot \semi \Pi \vdval v: \tau$ with $\tau \in \lits$.
\item We conclude via \textsc{TTree-Lit} that $\Pi \vdtree \mf{lit}\ v$
and we are done. 
\end{itemize}

\textbf{Case} \textsc{Init-Node}: \begin{itemize}
\item We have $\mc{D} \vdash \mf{tag}(v, \num{v_i}, \num{v_j})
  \init
  \pair{\mf{node}(\mf{id}, v, \num{v_i}, \num{t_j})}{\bigcup_j m_j}$.
\item Furthermore, for all $j$, we have (a) that $\mc{D} \vdash v_j \init \pair{t_j}{m_j}$.
\item By \emph{(iii)} and \textsc{TVal-Tag}, we have \begin{enumerate}[(a)]
  \stepcounter{enumi}
  \item $\cdot \semi \Pi \vdval v: \mf{String}$,
  \item $\cdot \semi \Pi \vdval v_i: \mf{Attr}$ for all $i$, and
  \item $\cdot \semi \Pi \vdval v_j: \mf{View}$ for all $j$.
\end{enumerate}
\item We \emph{iteratively} apply the inductive hypothesis for each $j$.
\begin{itemize}
  \item Let $j$ range from $0$ to $n$ (exclusive).
  \item Let $\Pi_0=\Pi$.
  \item At step $j$, we weaken (d) to get (e) that $\cdot \semi \Pi_j \vdval v_j: \mf{View}$.
  \begin{itemize}
    \item That $\Pi \po \Pi_j$ is given by reflexivity at $j=0$, 
    or the previous conclusion (f) \& transitivity of $\po$ otherwise.
    (We have a chain $\Pi_0 \po \Pi_1 \po \cdots \po \Pi_j$).
  \end{itemize}
  \item We apply the inductive hypothesis using (a), (e), and premise \emph{(ii)}.
  \item We get some $\Pi_{j+1}$ with:
  \begin{enumerate}[(a)]
    \setcounter{enumi}{5}
    \item $\Pi_j \po \Pi_{j+1}$,
    \item $\Pi_{j+1} \vdtree t_j$, and
    \item $\cdot \semi \Pi_{j+1} \semi \mc{D} \vdmstr m_{j}$.
  \end{enumerate}
\end{itemize}
\item Choose $\Pi'=\Pi_n$.
\item By (f) and transitivity of $\po$, we get $\Pi \po \Pi_n$, i.e. conclusion \emph{(i)}.
\item Since $\Pi_{j+1} \po \Pi_n$ for all $j$, weaken (g) and (h) to say
\begin{enumerate}[(a$'$)]
  \setcounter{enumi}{6}
  \item $\Pi_n \vdtree t_j$ and
  \item $\cdot \semi \Pi_{n} \semi \mc{D} \vdmstr m_j$.
\end{enumerate}
\item By the memory freshness lemma on (a), we get that all $p \in m_j$ are fresh,
i.e. all $m_j$ are disjoint.
\item Therefore, by the strong disjoint memory lemma,
we get by (h$'$) that
$\cdot \semi \Pi_n \vdmstr \bigcup_j m_j$, i.e. conclusion \emph{(iii)}.
\item Finally, we apply \textsc{TTree-Node} using (b) and (c) weakened on $\Pi$,
as well as (g$'$),
to get $\Pi_n \vdtree \mf{node}(\mf{id}, v, \num{v_i}, \num{t_j})$, 
i.e conclusion \emph{(ii)}, and we are done.
\end{itemize}

\textbf{Case} \textsc{Init-Comp}: \begin{itemize}
\item We start with $\mc{D} \vdash \pair{\mc{C}}{v_0}
  \init
  \pair{p}{m' \cup (m \mid m[p].\mf{child} = t)}$.
\item Let $\mc{D}[\mc{C}] = x.\hat{e}$ and
$m_0 = \{\mf{spec}: \pair{\mc{C}}{v_0}, \mf{st}: \emptyset, \mf{child}: \emptyset\}$.
\item We have the following premises:
\begin{enumerate}[(a)]
  \item $p$ is fresh,
  \item $m_0 \semi \hat{e}[v_0/x] \Topstep{\mf{Init}}{p} v \semi m$ (S3), and
  \item $\mc{D} \vdash v \init \pair{t}{m'}$.
\end{enumerate}
\item Let $\Pi_0=\Pi[p \mapsto \mc{C}]$; by freshness of $p$, $\Pi \po \Pi_0$.
Note that $\cdot \semi \Pi_0 \vdmwk m_0$ (S2) by definition.
\item By premise \emph{(ii)}, we get 
$[x \mapsto \Delta[\mc{C}]] \semi \cdot \semi \mc{C} \vdtop \hat{e}$.
\item Premise \emph{(iii)} gives the following derivation (bottom-up):
\begin{mathpar}
\inferrule*[right=TVal-Spec] {
  \inferrule*[right=TVal-Comp] {
    \inferrule*{(d)} {\Delta[\mc{C}] = \tau}
  } {
    \cdot \semi \Pi \vdval \mc{C}: \mf{Component}\ \tau
  }
  \and
  \inferrule*{(e)} {\cdot \semi \Pi \vdval v_0: \tau}
} {
  \cdot \semi \Pi \vdval \pair{\mc{C}}{v_0} : \mf{View}
}
\end{mathpar}
\item By (d), $[x \mapsto \tau] \semi \cdot \semi \mc{C} \vdtop \hat{e}$.
\item By (e), weakening, and substitution lemma,
$\cdot \semi \Pi \semi \mc{C} \vdtop \hat{e}[v/x]$.
\item We weaken this again to get $\cdot \semi \Pi_0 \semi \mc{C} \vdtop \hat{e}[v/x]$ (S1).
\item We can therefore apply the soundness of top-level expressions at initial phase
to (S1-3), and get that:
\begin{enumerate}[(a)]
  \setcounter{enumi}{5}
  \item $\cdot \semi \Pi_0 \vdval v: \mf{View}$, and
  \item $\cdot \semi \Pi_0 \vdmwk m$.
\end{enumerate}
\item We then apply the inductive hypothesis to (c) and (f) to get some $\Pi_1$ with:
\begin{enumerate}[(a)]
  \setcounter{enumi}{7}
  \item $\Pi_0 \po \Pi_1$,
  \stepcounter{enumi}
  \item $\Pi_1 \vdtree t$, and
  \item $\cdot \semi \Pi_1 \semi \mc{D} \vdmstr m'$.
\end{enumerate}
\item Choose this $\Pi_1$ as our $\Pi'$. We now need to show each of the conclusions.
\item That $\Pi \po \Pi'$ follows from (h), definition of $\Pi_0$,
and transitivity of $\po$. (Conclusion \emph{(i)} satisfied.)
\item That $\Pi' \vdtree p$ (conclusion \emph{(ii)}) 
is immediate from \textsc{TTree-Path} and $\Pi_0 \po \Pi'$.
\item We then need to show conclusion \emph{(iii)}, that 
$\cdot \semi \Pi' \semi \mc{D} \vdmstr m' \cup (m \mid m[p].\mf{child} = t)$.
\item Let $m_1 = m \mid m[p].\mf{child} = t$.
By (k) and the strong disjoint memory lemma, it suffices to show
that $\cdot \semi \Pi' \semi \mc{D} \vdmstr m_1$.
\item By definition of $\vdmstr$, we need the following sub-conclusions:
\begin{enumerate}[(\emph{iii}-a)]
  \item $\cdot \semi \Pi' \vdmwk m_1$,
  \item $\Delta \vDash \mc{D}$,
  \item for all $p \in \dom(m_1)$, for $\mc{D}[\Pi[p]]=x.\hat{e}$,
  $\mc{L}(\hat{e}) \subseteq \dom(m_1[p].\mf{st})$, and
  \item for all $p \in \dom(m_1)$, $\Pi' \vdtree m_1[p].\mf{child}$.
\end{enumerate}
\item (\emph{iii}-a) follows from (g) and weakening via (h).
\item (\emph{iii}-b) is immediate by premise \emph{(ii)}.
\item For (\emph{iii}-c) and (\emph{iii}-d), we apply the path preservation lemma
to (b) to say that $\dom(m_0) = \dom(m)$.
\item By definition of each of $m_0$ and $m_1$, we get $\dom(m_1)=\{p\}$.
\item Therefore, (\emph{iii}-d) is satisfied by (j).
\item Finally, the label initialization corollary gives us that\\
$\mc{L}(\hat{e}) \subseteq \dom(m[p].\mf{st})$, 
i.e. sub-conclusion (\emph{iii}-c), and we are done.
\end{itemize}
\end{proof}

% \subsubsection{Rerendering}

% Below is all old.

% \begin{lemma}[Check Lemma]
% If \begin{enumerate}
% \item $\mc{D} \semi m_\mf{old} \semi m_\mf{new} \vdash \mf{check}(v) = m$,
% \item $\Delta \vDash \mc{D}$,
% \item $\Pi \vDash_\Delta m_\mf{old}$,
% \item $\Pi \vDash_\Delta m_\mf{new}$, and
% \item $\Pi \vDash t$,
% \end{enumerate}
% then there exists some $\Pi'$ such that: \begin{enumerate}
% \item $\Pi \sqsubseteq \Pi'$ and
% \item $\Pi' \vDash_\Delta m'$.
% \end{enumerate}
% \end{lemma}

% \begin{proof}

% By induction on derivation of the judgment.

% \textbf{Case} \textsc{Check-Lit}: We have
% \begin{mathpar}
% \inferrule*[right=Check-Lit] { } {
%   \mc{D} \semi m_\mf{old} \semi m_\mf{new} \vdash \mf{check}(\mf{lit}\ v) 
%   = m_\mf{new}
% }
% \end{mathpar}

% Choose $\Pi'=\Pi$; both conclusions are trivial.

% \textbf{Case} \textsc{Check-Node}: We have
% \begin{mathpar}
% \inferrule*[right=Check-Node] {
%   (0 \leq j < n) \quad
%   \mc{D} \semi m_\mf{old} \semi m_j \vdash \mf{check}(t_j) = m_{j+1}
% } {
%   \mc{D} \semi m_\mf{old} \semi m_0 \vdash \mf{check}(\mf{node}(id, v, \num{v_i}, \num{t_j})) = m_n
% }
% \end{mathpar}

% Repeated application of the inductive hypothesis,
% obtaining $\Pi_j$ each time,
% gives some $\Pi_n$ with $\Pi \sqsubseteq \Pi_n$
% and $\Pi_n \vDash_\Delta m_n$.

% \textbf{Case} \textsc{Check-PathConst}: We have
% \begin{mathpar}
% \inferrule*[right=Check-PathConst] {
%   m_\mf{old}[p].\mf{st} = m_\mf{new}[p].\mf{st}\\
%   \mc{D} \semi m_\mf{old} \semi m_\mf{new} 
%   \vdash 
%   \mf{check}(m_\mf{old}[p].\mf{child}) 
%   = m
% } {
%   \mc{D} \semi m_\mf{old} \semi m_\mf{new} \vdash \mf{check}(p) 
%   = m
% }
% \end{mathpar}

% (That $m[p]$ are well-defined is given by premise 4,
% in this case and the below.)

% We simply apply the inductive hypothesis on the second premise
% to get our $\Pi'$ with $\Pi \sqsubseteq \Pi'$
% and $\Pi' \vDash_\Delta m$.

% \textbf{Case} \textsc{Check-PathDiff}: We have
% \begin{mathpar}
% \inferrule*[right=Check-PathDiff] { 
%   m_\mf{old}[p].\mf{st} \neq m_\mf{new}[p].\mf{st}
%   \and
%   m_\mf{new}[p].\mf{spec} = \pair{\mc{C}}{v}
%   \\
%   \mc{D}[\mc{C}] = x.\hat{e}
%   \\
%   m_\mf{new} \semi \hat{e}[v/x] \multistep{\mf{Succ}}{p} \eret{v'} \semi m
%   \\
%   \mc{D} \semi m \vdash \mf{init}(v') = \pair{t}{m'}
% } {
%   \mc{D} \semi m_\mf{old} \semi m_\mf{new} \vdash \mf{check}(p)
%    = m' \mid m'[p].\mf{child}=t
% }
% \end{mathpar}

% By premise 3, $\cdot \vdashv v: \Delta[\mc{C}]$,
% and by premise 2, $x: \Delta[\mc{C}] \vdtop \hat{e}$.
% By substitution lemma, $\cdot \vdtop \hat{e}[v/x]$.

% We now want to apply soundness of top-level expressions at $\phi=\mf{Succ}$,\\
% \hl{which still needs formulating},
% to say that $\cdot \vdval v': \mf{View}$.

% We can then apply the Initialization Lemma
% to get some $\Pi'$ with $\Pi \sqsubseteq \Pi'$,
% $\Pi' \vDash t$, and $\Pi' \vDash_\Delta m'$.
% It follows that $\Pi' \vDash_\Delta m' \mid m'[p].\mf{child} = t$,
% and we are done.

% \end{proof}

\section{$\lambda_{CC}$}

\subsection{Syntax}

Additions/changes from React-tRace Core are \hl{highlighted}.
Intermediate-only constructs are \grey{greyed}
\begin{align*}
\text{Value } v &::= x \mid () \mid \mf{true} \mid \mf{false} 
\mid s \mid \lambda x: \tau.e\\
&\mid \mf{text}\ v \mid \mf{tag}(v, \num{v}, \num{v})
\mid \mf{attr}(v, v) \mid \mf{onClick}\ v \mid \mathgrey{\ell}\\
\text{Expression } e &::= \eret{v} \mid \elet{x}{e}{e} 
\mid v\ v \mid \eif{v}{e}{e}\\
&\mid \mathhl{\eref{v}} \mid \mathhl{\eget{e}} \mid \mathhl{\eset{v}{v}}\\
\text{Type } \tau &::= \one \mid \mf{Bool} \mid \mathhl{\tau \rightarrow \tau}
\mid \mf{View} \mid \mf{Attr} \mid \mathhl{\mf{Ref}\ \tau}\\
\end{align*}
Internal representations:
\begin{align*}
\text{Type context } \Gamma &::= \map{x}{\tau}\\
\text{Store } \sigma &::= \map{\ell}{v}\\
\text{Tree }t &::= \mf{lit}\ v \mid \mf{node}(id, v, \num{v}, \num{t})\\
\text{Mode }\mu &::= \mf{start}(e) \mid \mf{idle} \mid \mf{render}
\end{align*}

\subsection{Typing}

Non-standard rules are \hl{highlighted}, although on the whole,
they are the same as in React-tRace Core. (Most relevant are the omissions.)

\pagebreak

\subsubsection{Value Typing}

Judgment: $\Gamma \vdashv v: \tau$

\begin{mathpar}
\inferrule*[right=TVal-Var] {
  x : \tau \in \Gamma
} {
  \Gamma \vdashv x: \tau
}
\and
\inferrule*[right=TVal-Unit] { } {
  \Gamma \vdashv () : \one
}
\and
\inferrule*[right=TVal-True] { } {
  \Gamma \vdashv \mf{true} : \mf{Bool}
}
\and
\inferrule*[right=TVal-False] { } {
  \Gamma \vdashv \mf{false} : \mf{Bool}
}
\and
\inferrule*[right=TVal-Str] { } {
  \Gamma \vdashv s : \mf{String}
}
\and
\inferrule*[right=TVal-Lam] {
  \Gamma, x: \tau_1 \vdashe e : \tau_2
} {
  \Gamma \vdashv \lambda x: \tau_1.e: \tau_1 \rightarrow \tau_2
}
\and
\inferrule*[right=\hl{TVal-Text}] {
  \Gamma \vdashv v: \tau
  \and
  \tau \in \{\mf{Bool},\mf{String}\}
} {
  \Gamma \vdashv \mf{text}\ v: \mf{View}
}
\and
\inferrule*[right=\hl{TVal-Tag}] {
  \Gamma \vdashv v: \mf{String}
  \and
  (\forall i) \
  \Gamma \vdashv v_i : \mf{Attr}
  \and
  (\forall j) \
  \Gamma \vdashv v_j: \mf{View}
}{
  \Gamma \vdashv \mf{tag}(v, \num{v_i}, \num{v_j}): \mf{View}
}
\and
\inferrule*[right=\hl{TVal-Attr}] {
  \Gamma \vdashv v_1: \mf{String}
  \and
  \Gamma \vdashv v_2: \mf{String}
} {
  \Gamma \vdashv \mf{attr}(v_1, v_2): \mf{Attr}
}
\and
\inferrule*[right=\hl{TVal-OnClick}] {
  \Gamma \vdashv v: \one \rightarrow \one
} {
  \Gamma \vdashv \mf{onClick}\ v: \mf{Attr}
}
\end{mathpar}

\pagebreak

\subsubsection{Expression Typing}

Additions relative to React-tRace core are \hl{highlighted},
although these are just standard rules for references.

A top-level program should have type $\one \rightarrow \mf{View}$.

Judgment: $\Gamma \vdashe e: \tau$

\begin{mathpar}
\inferrule*[right=TExp-Ret] {
  \Gamma \vdashv v: \tau
} {
  \Gamma \vdashe \eret{v}: \tau
}
\and
\inferrule*[right=TExp-Let] {
  \Gamma \vdashe e_1: \tau_1
  \and
  \Gamma, x: \tau_1 \vdashe e_2: \tau_2
} {
  \Gamma \vdashe \elet{x}{e_1}{e_2}: \tau_2
}
\and
\inferrule*[right=TExp-App] {
  \Gamma \vdashv v_1: \tau_1 \rightarrow \tau_2
  \and
  \Gamma \vdashv v_2: \tau_2
} {
  \Gamma \vdashe v_1\ v_2: \tau_2
}
\and
\inferrule*[right=TExp-If] {
  \Gamma \vdashv v : \mf{Bool}
  \and
  \Gamma \vdashe e_1 : \tau
  \and
  \Gamma \vdashe e_2 : \tau
} {
  \Gamma \vdashe \eif{v}{e_1}{e_2} : \tau
}
\and
\inferrule*[right=\hl{TExp-Ref}] {
  \Gamma \vdashv v: \tau
} {
  \Gamma \vdashe \eref{v}: \mf{Ref}\ \tau
}
\and
\inferrule*[right=\hl{TExp-Get}] {
  \Gamma \vdashv v : \mf{Ref}\ \tau
} {
  \Gamma \vdashe \eget{v} : \tau
}
\and
\inferrule*[right=\hl{TExp-Set}] {
  \Gamma \vdashv v_1: \mf{Ref}\ \tau
  \and
  \Gamma \vdashv v_2: \tau
} {
  \Gamma \vdashe \eset{v_1}{v_2}: \one
}
\end{mathpar}

\subsection{Operational Semantics}

\subsubsection{Expression Evaluation}

Judgment: $\sigma, e \mapsto e' \semi \sigma'$
\begin{mathpar}
\inferrule*[right=E-Let] {
  \sigma \semi e_1 \mapsto e_1' \semi \sigma'
} {
  \sigma \semi \elet{x}{e_1}{e_2} \mapsto \elet{x}{e_1'}{e_2} \semi \sigma'
}
\end{mathpar}
\begin{align*}
\sigma \semi \elet{x}{\eret{v}}{e} 
&\mapsto e[v/x] \semi \sigma
&&\textsc{E-Ret}\\
\sigma \semi (\lambda x.e) v
&\mapsto e[v/x] \semi \sigma
&&\textsc{E-Beta}\\
\sigma \semi \eif{\mf{true}}{e_1}{e_2} 
&\mapsto e_1 \semi \sigma
&&\textsc{E-IfTrue}\\
\sigma \semi \eif{\mf{false}}{e_1}{e_2} 
&\mapsto e_2 \semi \sigma
&&\textsc{E-IfFalse}\\
\sigma \semi \eref{v} 
&\mapsto \eret{\ell} \semi \sigma[\ell \mapsto v] \quad (\ell \text{ fresh})
&&\textsc{E-Ref}\\
\sigma \semi \eget{\ell}
&\mapsto \eret{\sigma[\ell]} \semi \sigma
&&\textsc{E-Get}\\
\sigma \semi \eset{\ell}{v}
&\mapsto \eret{()} \semi \sigma[\ell \mapsto v]
&&\textsc{E-Set}
\end{align*}

\subsubsection{Tree Annotation}

Judgment: $\mf{annotate}(v) = t$

\begin{mathpar}
\inferrule*[right=Annot-Text] { } {
  \mf{annotate}(\mf{text}\ v) = \mf{lit}\ v
}
\and
\inferrule*[right=Annot-Tag] {
    id \text{ fresh}
    \and
    (\forall j)\
    \mf{annotate}(v_j) = t_j
} {
  \mf{annotate}(\mf{tag}(v, \num{v_i}, \num{v_j})) 
  = \mf{node}(id, v, \num{v_i}, \num{t_j})\\
}
\end{mathpar}

\subsubsection{Handler Search}

Judgment: $\mf{handlers}(t, id) = \num{v}$

\begin{mathpar}
\inferrule*[right=Handlers-Lit] { } {
  m \vdash \mf{handlers}(\mf{lit}\ v, id) = \{\}
}
\and
\inferrule*[right=Handlers-Tgt] { } {
  m \vdash \mf{handlers}(\mf{node}(id, v, \num{v_i}, \num{v_j}), id)
  = \{v_h \mid \mf{onClick}\ v_h \in \num{v_i}\}
}
\and
\inferrule*[right=Handlers-Node] {
  id \neq id'
  \and
  (\forall j)\ 
  m \vdash \mf{handlers}(v_j, id) = h_j
} {
  m \vdash \mf{handlers}(\mf{node}(id', v, \num{v_i}, \num{v_j}), id)
  = \bigcup_{\forall j} h_j
}
\end{mathpar}

\subsubsection{Reactive Semantics}

Judgment: $\four{\sigma}{t}{v}{\mu} \hookrightarrow \four{\sigma'}{t'}{v'}{\mu'}$

\begin{mathpar}
\inferrule*[right=R-Start] {
  \cdot \semi e \mapsto^* \eret{v} \semi \sigma
} {
  \four{\cdot}{\cdot}{\cdot}{\mf{start}(e)}
  \hookrightarrow
  \four{\sigma}{\cdot}{v}{\mf{render}}
}
\and
\inferrule*[right=R-Render] {
  \sigma \semi v_r\; () \mapsto^* \eret{v} \semi \sigma'
  \and
  \mf{annotate}(v) = t
} {
  \four{\sigma}{\_}{v_r}{\mf{render}}
  \hookrightarrow
  \four{\sigma'}{t}{v_r}{\mf{idle}}
}
\and
\inferrule*[right=R-Event] {
  \exists id
  \and
  \mf{handlers}(t, id) = \num{v_i}
  \\
  (0 \leq i < n) \quad
  \sigma_i \semi v_i\; () \mapsto^* \eret{()} \semi \sigma_{i+1}
} {
  \four{\sigma_0}{t}{v_r}{\mf{idle}}
  \hookrightarrow
  \four{\sigma_n}{t}{v_r}{\mf{render}}
}
\end{mathpar}

\subsection{Type Soundness (Expressions)}

\subsubsection{Store Context}

We introduce the additional context $\Sigma=\map{\ell}{\tau}$
to type standalone locations.

Define $\Gamma \semi \Sigma \vDash \sigma$ to say that 
$\mf{dom}(\Sigma) = \mf{dom}(\sigma)$ and
$\Gamma \semi \Sigma \vdashv \sigma[\ell]: \Sigma[\ell]$.

Further define $\Sigma \sqsubseteq \Sigma'$ to hold when,
for all $\ell \in \Sigma$, both $\ell \in \Sigma'$ and $\Sigma(\ell) = \Sigma'(\ell)$.
Overload $\sqsubseteq$ analogously for type contexts $\Gamma$.

\subsubsection{Typing Rules}

We extend all typing rules with $\Sigma$, and introduce
a new rule \textsc{TVal-Loc} to use it.

Augmented value judgment: $\Gamma \semi \Sigma \vdashv v: \tau$

\begin{mathpar}
\inferrule*[right=\hl{TVal-Loc}] {
  \ell : \tau \in \Sigma
} {
  \Gamma \semi \Sigma \vdashv \ell: \mf{Ref}\ \tau
}
\and
\inferrule*[right=TVal-Var] {
  x : \tau \in \Gamma
} {
  \Gamma \semi \Sigma \vdashv x: \tau
}
\and
\inferrule*[right=TVal-Unit] { } {
  \Gamma \semi \Sigma \vdashv () : \one
}
\and
\inferrule*[right=TVal-True] { } {
  \Gamma \semi \Sigma \vdashv \mf{true} : \mf{Bool}
}
\and
\inferrule*[right=TVal-False] { } {
  \Gamma \semi \Sigma \vdashv \mf{false} : \mf{Bool}
}
\and
\inferrule*[right=TVal-Str] { } {
  \Gamma \semi \Sigma \vdashv s : \mf{String}
}
\and
\inferrule*[right=TVal-Lam] {
  \Gamma, x: \tau_1 \semi \Sigma \vdashe e : \tau_2
} {
  \Gamma \semi \Sigma \vdashv \lambda x: \tau_1.e: \tau_1 \rightarrow \tau_2
}
\and
\inferrule*[right=TVal-Text] {
  \Gamma \semi \Sigma \vdashv v: \tau
  \and
  \tau \in \{\mf{Bool},\mf{String}\}
} {
  \Gamma \semi \Sigma \vdashv \mf{text}\ v: \mf{View}
}
\and
\inferrule*[right=TVal-Tag] {
  \Gamma \semi \Sigma \vdashv v: \mf{String}
  \\
  (\forall i) \
  \Gamma \semi \Sigma \vdashv v_i : \mf{Attr}
  \and
  (\forall j) \
  \Gamma \semi \Sigma \vdashv v_j: \mf{View}
}{
  \Gamma \semi \Sigma \vdashv \mf{tag}(v, \num{v_i}, \num{v_j}): \mf{View}
}
\and
\inferrule*[right=TVal-Attr] {
  \Gamma \semi \Sigma \vdashv v_1: \mf{String}
  \and
  \Gamma \semi \Sigma \vdashv v_2: \mf{String}
} {
  \Gamma \semi \Sigma \vdashv \mf{attr}(v_1, v_2): \mf{Attr}
}
\and
\inferrule*[right=TVal-OnClick] {
  \Gamma \semi \Sigma \vdashv v: \one \rightarrow \one
} {
  \Gamma \semi \Sigma \vdashv \mf{onClick}\ v: \mf{Attr}
}
\end{mathpar}

Augmented expression judgment: $\Gamma \semi \Sigma \vdashe e: \tau$

\begin{mathpar}
\inferrule*[right=TExp-Ret] {
  \Gamma \semi \Sigma \vdashv v: \tau
} {
  \Gamma \semi \Sigma \vdashe \eret{v}: \tau
}
\and
\inferrule*[right=TExp-Let] {
  \Gamma \semi \Sigma \vdashe e_1: \tau_1
  \and
  \Gamma, x: \tau_1 \semi \Sigma \vdashe e_2: \tau_2
} {
  \Gamma \semi \Sigma \vdashe \elet{x}{e_1}{e_2}: \tau_2
}
\and
\inferrule*[right=TExp-App] {
  \Gamma \semi \Sigma \vdashv v_1: \tau_1 \rightarrow \tau_2
  \and
  \Gamma \semi \Sigma \vdashv v_2: \tau_2
} {
  \Gamma \semi \Sigma \vdashe v_1\ v_2: \tau_2
}
\and
\inferrule*[right=TExp-If] {
  \Gamma \semi \Sigma \vdashv v : \mf{Bool}
  \and
  \Gamma \semi \Sigma \vdashe e_1 : \tau
  \and
  \Gamma \semi \Sigma \vdashe e_2 : \tau
} {
  \Gamma \semi \Sigma \vdashe \eif{v}{e_1}{e_2} : \tau
}
\and
\inferrule*[right=TExp-Ref] {
  \Gamma \semi \Sigma \vdashv v: \tau
} {
  \Gamma \semi \Sigma \vdashe \eref{v}: \mf{Ref}\ \tau
}
\and
\inferrule*[right=TExp-Get] {
  \Gamma \semi \Sigma \vdashv v : \mf{Ref}\ \tau
} {
  \Gamma \semi \Sigma \vdashe \eget{v} : \tau
}
\and
\inferrule*[right=TExp-Set] {
  \Gamma \semi \Sigma \vdashv v_1: \mf{Ref}\ \tau
  \and
  \Gamma \semi \Sigma \vdashv v_2: \tau
} {
  \Gamma \semi \Sigma \vdashe \eset{v_1}{v_2}: \one
}
\end{mathpar}

\subsubsection{Unproved Lemmas}

We assume that the typical structural rules for intuitionistic calculi,
i.e. weakening, contraction, and exchange, hold on both $\Gamma$ and $\Sigma$.
For example:

\begin{lemma}[Weakening $\Gamma$ in $\vdashv$]
If $\Gamma \semi \Sigma \vdashv v: \tau$
then $\Gamma' \semi \Sigma \vdashv v: \tau$ 
for any $\Gamma'$ such that $\Gamma \sqsubseteq \Gamma'$.
\end{lemma}

\begin{proof}
By induction on the derivation of the typing judgment.
\end{proof}

We also assume the following canonical forms lemma on values:

\begin{lemma}[Canonical Forms]
If $\cdot \semi \Sigma \vdashv v: \tau$, then
\begin{itemize}
\item If $\tau=\mf{Bool}$, then $v=\mf{true}$ or $v=\mf{false}$.
\item If $\tau=\one$, then $v=()$.
\item If $\tau=\mf{String}$, then $v=s$ for some string $s$.
\item If $\tau=\tau_1 \rightarrow \tau_2$, then $v=\lambda x: \tau_1.e$.
\item If $\tau=\mf{Attr}$, then $v=\mf{attr}(v_1, v_2)$ or $v=\mf{onClick} v$.
\item If $\tau=\mf{View}$, then $v=\mf{text}\ v$ or $v=\mf{tag}(v, \num{v_i}, \num{v_j})$. 
\item If $\tau=\mf{Ref}\ \tau'$, then $v=\ell$.
\end{itemize}
\end{lemma}

\subsubsection{Substitution}

\begin{lemma}[Type preservation under substitution]
\label{sub-val}
If $\Gamma \semi \Sigma \vdashv v: \tau$, then the following two conditionals hold:
\begin{enumerate}[(i)]
\item If $\Gamma, x: \tau \semi \Sigma \vdashv v': \tau'$, 
then $\Gamma \semi \Sigma \vdashv v'[v/x]: \tau'$.
\item If $\Gamma, x: \tau \semi \Sigma \vdashe e: \tau'$, 
then $\Gamma \semi \Sigma \vdashe e[v/x]: \tau'$.
\end{enumerate}
\end{lemma}

\begin{proof}
By mutual induction on \emph{(i)} and \emph{(ii)}. For each,
we induct over the derivation of the respective typing judgment
in the antecedent.

\textbf{Case} \textsc{TVal-Var}:
We have $\Gamma, x: \tau \semi \Sigma \vdashv x: \tau'$.
Applying substitution, we want to show 
$\Gamma \semi \Sigma \vdashv v: \tau'$, which is exactly our first premise
(with $\tau'=\tau$).

\textbf{Case} \textsc{TVal-Unit}:
We have $\Gamma, x: \tau \semi \Sigma \vdashv (): \one$.
Trivial, since $()[v/x]=()$ and $\Gamma \semi \Sigma \vdashv (): \one$ axiomatically.
We elide analogous cases for boolean and string constants.

\textbf{Case} \textsc{TVal-Loc}:
We have $\Gamma, x: \tau \semi \Sigma \vdashv \ell: \tau$.
By inversion, $\ell: \tau \in \Sigma$,
and since $\ell[v/x]=\ell$, we get $\Gamma \semi \Sigma \vdashv \ell: \tau$.

\textbf{Case} \textsc{TVal-Lam}:
We have $\Gamma, x: \tau \semi \Sigma \vdashv \lambda y.e : \tau_1 \rightarrow \tau_2$.
First suppose $y=x$: by definition of substitution, $(\lambda x.e)[v/x] = \lambda x.e$,
and the conclusion follows from inversion \& reapplication of \textsc{TVal-Lam},
noting that $x: \tau$ is overridden by $x: \tau_1$ in the premise.

If $y \neq x$, then $(\lambda y.e)[v/x] = \lambda y.(e[v/x])$,
and inversion gives us $\Gamma, x: \tau, y: \tau_1 \semi \Sigma \vdashe e: \tau_2$.
Applying case \emph{(ii)} of the inductive hypothesis, alongside exchange,
gives that $\Gamma, y: \tau_1 \semi \Sigma \vdashe e[v/x]: \tau_2$.
We conclude $\Gamma \semi \Sigma \vdashv (\lambda y.e) [v/x]: \tau_1 \rightarrow \tau_2$
by reapplication of \textsc{TVal-Lam}.

\textbf{Case} \textsc{TVal-Tag}:
We have  $\Gamma, x: \tau \semi \Sigma \vdashv 
\mf{tag}(v', \num{v_i}, \num{v_j}): \mf{View}$.
By inversion, $\Gamma, x: \tau \semi \Sigma \vdashv v': \mf{String}$,
$\Gamma, x: \tau \semi \Sigma \vdashv v_i: \mf{Attr}$,
and $\Gamma, x: \tau \semi \Sigma \vdashv v_j: \mf{View}$
for all $i, j$.
We apply case \emph{(i)} of the inductive hypothesis to each, to obtain
$\Gamma \semi \Sigma \vdashv v'[v/x]: \mf{String}$,
$\Gamma \semi \Sigma \vdashv v_i[v/x]: \mf{Attr}$,
and $\Gamma \semi \Sigma \vdashv v_j[v/x]: \mf{View}$, respectively. 
By \textsc{TVal-Tag} and definition of substitution on $\mf{tag}$,
we conclude $\Gamma \semi \Sigma \vdashv 
\mf{tag}(v', \num{v_i}, \num{v_j})[v/x]: \mf{View}$.
We elide the simpler but analogous cases for data constructors 
\textsf{text}, \textsf{attr}, and \textsf{onClick}.

\textbf{Case} \textsc{TExp-Ret}:
We have $\Gamma, x: \tau \semi \Sigma \vdashe \eret{v'}: \tau'$.
By inversion, $\Gamma, x: \tau \semi \Sigma \vdashv v': \tau'$.
Case \emph{(i)} of the inductive hypothesis gives $\Gamma \semi \Sigma \vdashv v'[v/x]: \tau'$,
and reapplication \& definition of substitution let us conclude
$\Gamma \semi \Sigma \vdashe (\eret{v'})[v/x]: \tau'$.

\textbf{Case} \textsc{TVal-Let}:
$\Gamma, x: \tau \semi \Sigma \vdashe
\elet{y}{e_1}{e_2}: \tau_2$. Inversion gives 
$\Gamma, x: \tau \semi \Sigma \vdashe e_1: \tau_1$.
We have two subcases, when $y=x$ and when not.
In either case, $\Gamma \semi \Sigma \vdashe e_1[v/x] : \tau_1$
by inductive hypothesis.

If $y=x$, then $(\elet{y}{e_1}{e_2})[v/x]=\elet{y}{e_1[v/x]}{e_2}$.
We reapply \textsc{TExp-Let} with the second premise obtained via inversion and
we are done. (Like the $\lambda$ case, we note the overwriting of $x: \tau$ in
the second premise.)

If $y \neq x$, then we apply the inductive hypothesis to the second premise,
giving $\Gamma, x: \tau, y: \tau_1 \semi \Sigma \vdashe e_2: \tau_2$, and
by inductive hypothesis and exchange, $\Gamma, y: \tau_1 \vdashe e_2[v/x]: \tau_2$.
Reapplication of \textsc{TExpr-Let} and definition of substitution give
$\Gamma \semi \Sigma \vdashe (\elet{y}{e_1}{e_2})[v/x]: \tau_2$.

We elide the remaining cases for expressions, which are analogous to the above.

\end{proof}

\subsubsection{Progress}

\begin{theorem}[Progress of expressions]
If $\cdot \semi \Sigma \vdashe e: \tau$, then either:
\begin{enumerate}[(i)]
\item $e = \eret{v}$, or
\item for all $\sigma$ such that $\cdot \semi \Sigma \vDash \sigma$, 
there exist some $e'$ and $\sigma'$ such that 
$\sigma \semi e \mapsto e' \semi \sigma'$.
\end{enumerate}
\end{theorem}

\begin{proof}
By induction on the derivation of the typing judgment.

\textbf{Case} \textsc{TExp-Ret}: 
We have $\cdot \semi \Sigma \vdashe \eret{v} \semi \tau$.
The theorem holds trivially by case \emph{(i)}.

\textbf{Case} \textsc{TExp-Let}:
We have $\cdot \semi \Sigma \vdashe \elet{x}{e_1}{e_2}: \tau_2$.
By inversion, $\cdot \semi \Sigma \vdashe e_1: \tau_1$
and $x: \tau_1 \semi \Sigma \vdashe e_2: \tau_2$.
By inductive hypothesis on the first premise,
we have two subcases on $e_1$: either $e_1=\eret{v}$ 
or $\sigma \semi e_1 \mapsto e_1' \semi \sigma'$
(for $\cdot \semi \Sigma \vDash \sigma$).

If $e_1=\eret{v}$, then by \textsc{E-Ret},
we have $\sigma, \elet{x}{\eret{v}}{e_2} \mapsto e_2[v/x] \semi \sigma$
for any $\sigma$.

If instead $\sigma \semi e_1 \mapsto e_1' \semi \sigma'$,
by \textsc{E-Let}, we can conclude that 
$\sigma \semi \elet{x}{e_1}{e_2} \mapsto \elet{x}{e_1'}{e_2} \semi \sigma'$.

Thus, in either subcase, $\elet{x}{e_1}{e_2}$ steps given a proper $\sigma$,
so the case holds.

\textbf{Case} \textsc{TExp-App}:
We have $\cdot \semi \Sigma \vdashe v_1\ v_2: \tau_2$.
By inversion, $\cdot \semi \Sigma \vdashv v_1: \tau_1 \rightarrow \tau_2$
and $\cdot \semi \Sigma \vdashv v_2: \tau_1$.
By the canonical forms lemma, $v_1=\lambda x.e$,
and so we apply rule \textsc{E-Beta}, i.e.
$\sigma \semi (\lambda x.e) v_2 \mapsto e[v_2/x] \semi \sigma$ for
any $\sigma$ such that $\cdot \semi \Sigma \vDash \sigma$.

\textbf{Case} \textsc{TExp-If}:
We have $\cdot \semi \Sigma \vdashe \eif{v}{e_1}{e_2} : \tau$.
By inversion, $\cdot \semi \Sigma \vdashv v: \mf{Bool}$,
and by canonical forms, either $v=\mf{true}$ or $v=\mf{false}$.
If $v=true$, then \textsc{E-IfTrue} says 
$\sigma \semi \eif{v}{e_1}{e_2} \mapsto e_1 \semi \sigma$,
and if $v=false$, then $\sigma \semi \eif{v}{e_1}{e_2} \mapsto e_2 \semi \sigma$
by \textsc{E-IfFalse}. In either case, we satisfy \emph{(ii)}.

\textbf{Case} \textsc{TExp-Ref}:
We have $\cdot \semi \Sigma \vdashe \eref{v} : \mf{Ref}\ \tau$.
Immediately apply \textsc{E-Ref} to get that 
$\sigma, \eref{v} \mapsto \eret{\ell} \semi \sigma[\ell \mapsto v]$,
which satisfies \emph{(ii)}.

\textbf{Case} \textsc{TExp-Get}:
We have $\cdot \semi \Sigma \vdashe \eget{v} : \tau$.
By inversion, $\cdot \semi \Sigma \vdashv v: \mf{Ref}\ \tau$,
and by canonical forms, $v=\ell$.
Again by inversion (on \textsc{TVal-Loc}), $\ell: \tau \in \Sigma$.
Since $\cdot \semi \Sigma \vDash \sigma$ for our arbitrary $\sigma$,
we have that $\sigma[\ell]$ exists.
Thus, we apply \textsc{E-Get} to say
$\sigma \semi \eget{v} \mapsto \eret{\sigma[\ell]} \semi \sigma$.

\textbf{Case} \textsc{TExp-Set}:
We have $\cdot \semi \Sigma \vdashe \eset{v_1}{v_2}: \one$.
By inversion, $\cdot \semi \Sigma \vdashv v_1: \mf{Ref}\ \tau$,
and by canonical forms, $v=\ell$.
Applying \textsc{E-Set} then gives 
$\sigma \semi \eset{v_1}{v_2} \mapsto \eret{()} \semi \sigma[\ell \mapsto v_2]$.

\end{proof}

\subsubsection{Preservation}

\begin{theorem}[Preservation of expressions]
If
\begin{enumerate}[(i)]
\item $\Gamma \semi \Sigma \vdashe e: \tau$,
\item $\Gamma \semi \Sigma \vDash \sigma$ for some $\sigma$, and
\item $\sigma \semi e \mapsto e' \semi \sigma'$ for some $e'$ and $\sigma'$,
\end{enumerate}
then there is some $\Sigma'$ such that:
\begin{itemize}
\item $\Sigma \sqsubseteq \Sigma'$,
\item $\Gamma \semi \Sigma' \vDash \sigma'$, and
\item $\Gamma \semi \Sigma' \vdashe e': \tau$.
\end{itemize}
\end{theorem}

\begin{proof}
By induction on derivation of \emph{(iii)}.

\textbf{Case} \textsc{E-Let}:
We have $\sigma \semi \elet{x}{e_1}{e_2} \mapsto \elet{x}{e_1'}{e_2} \semi \sigma'$.
By inversion on \textsc{E-Let}, we also have $\sigma \semi e_1 \mapsto e_1' \semi \sigma'$.
By inversion on \emph{(i)}, $\Gamma \semi \Sigma \vdashe e_1: \tau_1$.
By inductive hypothesis, there exists some $\Sigma'$
with $\Gamma \semi \Sigma' \vDash \sigma'$
and $\Gamma \semi \Sigma' \vdashe e_1: \tau$.
Since $\Sigma \sqsubseteq \Sigma'$, and $\Gamma, x: \tau_1 \semi \Sigma \vdashe e_2: \tau$,
we also get that $\Gamma, x: \tau_1 \semi \Sigma' \vdashe e_2: \tau_2$.
We apply \textsc{E-Let} to get $\Gamma \semi \Sigma' \vdashe \elet{x}{e_1'}{e_2}: \tau$.

\textbf{Case} \textsc{E-Ret}:
We have $\sigma \semi \elet{x}{\eret{v}}{e} \mapsto e[v/x] \semi \sigma$.
Inversion on \emph{(i)} gives that $\Gamma \semi \Sigma \vdashv v: \tau_1$ and
$\Gamma, x: \tau_1 \semi \Sigma \vdashe e: \tau$. We conclude
$\Gamma \semi \Sigma \vdashe e[v/x]: \tau$ via the substitution lemma.

\textbf{Case} \textsc{E-Beta}:
We have $\sigma \semi (\lambda x: \tau_1.e)v \mapsto e[v/x] \semi \sigma$.
Inversion on \emph{(i)} gives $\Gamma, x: \tau_1 \semi \Sigma \vdashe e: \tau$
and $\Gamma \semi \Sigma \vdashv v: \tau_1$. We again get
$\Gamma \semi \Sigma \vdashe e[v/x]: \tau$ via the substitution lemma.

\textbf{Case} \textsc{E-IfTrue}:
We have $\sigma \semi \eif{\mf{true}}{e_1}{e_2} \mapsto e_1 \semi \sigma$.
Immediate by inversion on \emph{(i)}. Case for \textsc{E-IfFalse} is analogous.

\textbf{Case} \textsc{E-Ref}:
We have $\sigma \semi \eref{v} \mapsto \eret{\ell} \semi \sigma[\ell \mapsto v]$
(for $\ell$ fresh).
By \emph{(i)} and enumeration of rules, 
$\Gamma \semi \Sigma \vdashe \eref{v}: \mf{Ref}\ \tau$.
Inversion gives $\Gamma \semi \Sigma \vdashv v: \tau$.

Choose $\Sigma' = \Sigma[\ell \mapsto \tau]$; 
since $\ell$ is fresh, clearly $\Sigma \sqsubseteq \Sigma'$.
Since both $\Sigma'$ and $\sigma'$ are extensions of
$\Sigma$ and $\sigma$ by fresh label $\ell$, by definition of \emph{(ii)},
to show $\Gamma \semi \Sigma' \vDash \sigma'$,
it suffices to show $\cdot \semi \Sigma' \vdashv \sigma'[\ell]: \Sigma'[\ell]$.
Now since $\sigma'[\ell]=v$ and we said $\Sigma'[\ell] = \tau$,
we need only apply the proposition from the previous paragraph.

We also need to show that $\Gamma \semi \Sigma' \vdashe \eret{\ell} : \tau$.
This follows by \textsc{TExp-Ret}, \textsc{TVal-Loc}, and $\Sigma'[\ell] = \tau$.

\textbf{Case} \textsc{E-Get}:
We have $\sigma \semi \eget{\ell} \mapsto \eret{\sigma[\ell]} \semi \sigma$.

Choose $\Sigma'=\Sigma$; 
trivially $\Gamma \semi \Sigma' \vDash \sigma$ and $\Sigma \sqsubseteq \Sigma'$.

By inversion on \emph{(i)}, $\Gamma \semi \Sigma \vdashv \ell: \mf{Ref}\ \tau$
and $\ell: \tau \in \Sigma$.
We can then apply \textsc{TVal-Loc} and \textsc{TExp-Ret}
to get $\Gamma \semi \Sigma \vdashe \eret{\ell}: \tau$.

\textbf{Case} \textsc{E-Set}:
We have $\sigma \semi \eset{\ell}{v} \mapsto \eret{()} \semi \sigma[\ell \mapsto v]$.

Again choose $\Sigma'=\Sigma$. 
It is easy to show $\Gamma \semi \Sigma \vdashe \eret{()} : \one$,
and by enumeration of rules and \emph{(i)} 
we have $\Gamma \semi \Sigma \vdashe \eset{\ell}{v}: \one$.

We also need to show $\Gamma \semi \Sigma \vDash \sigma[\ell \mapsto v]$.
By \emph{(ii)}, this holds for all $\ell' \neq \ell$.
For $\ell$, we want to show $\cdot \semi \Sigma \vdashv \sigma'[\ell]: \Sigma[\ell]$,
i.e. that $\cdot \semi \Sigma \vdashv v: \Sigma[\ell]$.
Inversion on \emph{(i)} gives us both that $\Sigma[\ell] = \tau$
and that $\Gamma \semi \Sigma \vdashv v: \tau$.
\end{proof}

\subsubsection{Soundness}

\begin{theorem}[Type soundness for expressions]
If
\begin{itemize}
\item $\cdot \semi \Sigma \vdashe e: \tau$,
\item $\Sigma \vDash \sigma$ for arbitrary $\sigma$,
\item $\sigma \semi e \mapsto^* e' \semi \sigma'$ for some $e'$ and $\sigma'$,
\item and $e' \not \mapsto$ (i.e. $e'$ cannot step further),
\end{itemize}
then $e' = \eret{v}$, and for some $\Sigma'$ with $\Sigma \subseteq \Sigma'$
and $\Sigma' \vDash \sigma'$, we get $\cdot \semi \Sigma' \vdashv v: \tau$.
\end{theorem}

\begin{proof}
Follows directly from progess \& preservation.
\end{proof}

\begin{corollary}
If $\cdot \semi \cdot \vdashe e: \tau$ 
and $\cdot \semi e \mapsto^* e' \semi \sigma$
(and $e' \not \mapsto$),
then $e' = \eret{v}$,
and for any $\Sigma$ with $\Sigma \vDash \sigma$,
we get $\cdot \semi \Sigma \vdashv v: \tau$.
\end{corollary}

\subsection{Configuration Typing}



\section{Translation}

From React-tRace Core into $\lambda_{CC}$.

This requires the addition of (mutual) recursion into $\lambda_{CC}$,
which is entirely standard.

\subsection{Types}
Judgment: $\llrr{\tau_{RTC}} = \tau_{CC}$

\begin{align*}
\llrr{\one} &= \one\\
\llrr{\mf{Bool}} &= \mf{Bool}\\
\llrr{\mf{View}} &= \one \rightarrow \mf{View}\\
\llrr{\mf{Attr}} &= \mf{Attr}\\
\llrr{\tau_1 \xrightarrow{E} \tau_2} &= \llrr{\tau_1} \rightarrow \llrr{\tau_2}\\
\llrr{\mf{Component}\ \tau} &= \llrr{\tau} \rightarrow \llrr{\mf{View}}\\
\llrr{\mf{Setter}\ \tau} &= \llrr{\tau} \rightarrow \one
\end{align*}

\subsection{Values}

Judgment: $\llrr{v_{RTC}} = v_{CC}$
\begin{align*}
\llrr{x} &= x\\
\llrr{()} &= ()\\
\llrr{\mf{true}} &= \mf{true}\\
\llrr{\mf{false}} &= \mf{false}\\
\llrr{s} &= s\\
\llrr{\lambda x: \tau . e} & = \lambda x: \llrr{\tau}.\llrr{e}\\
\llrr{\mf{text}\ v} &= \mf{text}\ \llrr{v}\\
\llrr{\mf{tag}(v, \num{v_i}, \num{v_j})} 
&= \mf{tag}(\llrr{v}, \num{\llrr{v_i}}, \num{\llrr{v_j}})\\
\llrr{\mf{attr}(v_1, v_2)} &= \mf{attr}(\llrr{v_1}, \llrr{v_2})\\
\llrr{\mf{onClick}\ v} &= \mf{onClick}\ \llrr{v}\\
\llrr{\mc{C}} &= x_\mc{C}
\end{align*}

\subsection{Expressions}
Judgment: $\llrr{e_{RTC}} = e_{CC}$
\begin{align*}
\llrr{\eret{v}} &= \eret{\llrr{v}}\\
\llrr{\elet{x}{e_1}{e_2}} &= \elet{x}{\llrr{e_1}}{\llrr{e_2}}\\
\llrr{v_1\ v_2} &= \llrr{v_1}\ \llrr{v_2}\\
\llrr{\eif{v}{e_1}{e_2}} &= \eif{\llrr{v}}{\llrr{e_1}}{\llrr{e_2}}\\
\end{align*}

\subsection{Top-level Expressions}
Judgment: $\Theta \vdash \llrr{\hat{e}_{RTC}} = \hat{e}_{CC}$
where $\Theta ::= \cdot \mid (x, \ell), \Theta$

Also define $\Theta - x = \Theta \setminus \{x, \ell\}$ for any $\ell$
\begin{mathpar}
\inferrule*{
  \Theta - x \vdash \llrr{\hat{e}} = e'
} {
  \Theta \vdash \llrr{\elet{x}{e}{\hat{e}}} = \elet{x}{e}{e'}
}
\and
\inferrule*{
  \ell_x\ \mf{fresh}
  \and
  \Theta, (x, \ell_x) \vdash \llrr{\hat{e}} = e'
} {
  \Theta \vdash \llrr{\estate{x}{\ell}{v}{\hat{e}}} =\\
  \elet{\ell_x}{\eref{\llrr{v}}}{
    \elet{x_\mf{set}}{\lambda y: ?.\eset{\ell_x}{y}}{e'}
  }
}
\and
\inferrule*{
  \Theta \vdash \mf{deref}(\llrr{e}) = e'
} {
  \Theta \vdash \llrr{\hat{e}} = e'
}
\and
\inferrule*{
  \Theta \vdash \mf{deref}(e)
} {
  \Theta, (x, \ell_x) \vdash \mf{deref}(e) 
  = \elet{x}{\eget{\ell_x}}{e'}
}
\and
\inferrule*{ } {
  \cdot \vdash \mf{deref}(e) = e
}
\end{mathpar}

\subsection{Programs}
Judgment: $\Omega \vdash \llrr{P} = e$ 
where $\Omega = \cdot \mid \four{\mc{C}}{x}{\tau}{\hat{e}}, \Omega$
\begin{mathpar}
\inferrule*{
  \Omega \vdash \llrr{P} = e'
} {
  \Omega \vdash \llrr{\elet{x}{e}{P}} = \elet{x}{\llrr{e}}{e'}
}
\and
\inferrule*{
  \Omega, \four{\mc{C}}{x}{\tau}{\hat{e}} \vdash \llrr{P} = e'
} {
  \Omega \vdash \llrr{\elet{\mc{C}(x: \tau)}{\hat{e}}{P}} = e'
}
\and
\inferrule*{
  \forall \four{\mc{C}_i}{x_i}{\tau_i}{\hat{e}_i} \in \Omega
  \qquad
  \cdot \vdash \llrr{\hat{e}_i} = e_i
} {
  \Omega \vdash \erun{e} 
  = \eletrec{\num{\mc{C}_i: \llrr{\mf{Component}\ \tau_i}}}
  {\num{\lambda x_i: \llrr{\tau_i} . e_i}}{\llrr{e}}
}
\end{mathpar}

\section{$\lambda_{RTC}$-Anon (WIP)}

Introduces \emph{anonymous components},
removing the top-level restriction.

We restrict the number of mutually recursive functions/components
to $1$ for brevity, although this can (probably) be extended appropriately.

\subsection{Syntax}

\begin{align*}
\text{Value } v &::= x \mid () \mid \mf{true} \mid \mf{false} \mid s\\
&\mid \mathhl{\vrec{f}{x}{\tau}{e}} \mid \mathhl{\vcomp{\mc{C}}{x}{\tau}{e}}\\
&\mid \mf{text}\ v \mid \mf{tag}(v, \num{v}, \num{v})
\mid \mf{attr}(v, v) \mid \mf{onClick}\ v\\
&\mid \mathgrey{\four{\mc{C}}{v}{\tau}{x.e}} \mid \mathgrey{\vset{\ell}{p}}\\
\text{Expression } e &::= \eret{v} \mid \elet{x}{e}{e} \mid v\ v \mid \eif{v}{e}{e}\\
\text{Top-level expr }\hat{e} &::= e \mid \estate{x}{\ell}{v}{\hat{e}} \mid \elet{x}{e}{\hat{e}}\\
\end{align*}

Internal representations:
\begin{align*}
\text{Type context }\Gamma &::= \map{x}{\tau}\\
\text{Environment }\sigma &::= \map{x}{v}\\
\text{Phase }\phi &::= \mf{Init} \mid \mf{Succ}\\
\text{Tree }t &::= \mf{lit}\ v \mid \mf{node}(id, v, \num{v}, \num{t}) \mid p\\
\text{Realized component }\pi &::= \{\mf{spec}: \four{\mc{C}}{v}{\tau}{x.e}, \mf{st}: \map{\ell}{v}, \mf{child}: t\}\\
\text{Tree memory }m &::= \map{p}{\pi}\\
\text{Mode }\mu &::= \mf{start}(e) \mid \mf{idle} \mid \mf{rerender}(m)
\end{align*}

\subsection{Typing}

\subsubsection{Value Typing}

Judgment: $\Gamma \vdash v: \tau$

\begin{mathpar}
\inferrule*[right=TVal-Var] {
  x : \tau \in \Gamma
} {
  \Gamma \vdash x: \tau
}
\and
\inferrule*[right=TVal-Unit] { } {
  \Gamma \vdash () : \one
}
\and
\inferrule*[right=TVal-True] { } {
  \Gamma \vdash \mf{true} : \mf{Bool}
}
\and
\inferrule*[right=TVal-False] { } {
  \Gamma \vdash \mf{false} : \mf{Bool}
}
\and
\inferrule*[right=TVal-Str] { } {
  \Gamma \vdash s : \mf{String}
}
\and
\inferrule*[right=\hl{TVal-Fun}] {
  \Gamma, f: \tau_1 \xrightarrow{E} \tau_2, x: \tau_1 \vdash e : \tau_2 \mid E
} {
  \Gamma \vdash \vrec{f}{x}{\tau_1}{e}: \tau_1 \xrightarrow{E} \tau_2
}
\and
\inferrule*[right=\hl{TVal-Comp}] {
  \Gamma, \mc{C}: \mf{Component}\ \tau, x: \tau \Vdash \hat{e}: \mf{View}
} {
  \Gamma \vdash \vcomp{\mc{C}}{x}{\tau}{\hat{e}} : \mf{Component}\ \tau
}
\and
\inferrule*[right=TVal-Text] {
  \Gamma \vdash v: \tau
  \and
  \tau \in \{\mf{Bool}, \mf{String}\}
} {
  \Gamma \vdash \mf{text}\ v: \mf{View}
}
\and
\inferrule*[right=TVal-Tag] {
  \Gamma \vdash v: \mf{String}
  \and
  (\forall i)\ \Gamma \vdash v_i: \mf{Attr}
  \and
  (\forall j)\ \Gamma \vdash v_j: \mf{View}
} {
  \Gamma \vdash \mf{tag}(v, \num{v_i}, \num{v_j}): \mf{View}
}
\and
\inferrule*[right=TVal-Attr] {
  \Gamma \vdash v_k: \mf{String}
  \and
  \Gamma \vdash v_v: \mf{String}
} {
  \Gamma \vdash \mf{attr}(v_k, v_v): \mf{Attr}
}
\and
\inferrule*[right=TVal-OnClick] {
  \Gamma \vdash v: \one \xrightarrow{E} \one
} {
  \Gamma \vdash \mf{onClick}\ v: \mf{Attr}
}
\end{mathpar}

\subsubsection{Expression Typing}

Judgment: $\Gamma \vdash e : x \mid E$

\begin{mathpar}
\inferrule*[right=TExp-Ret] {
  \Gamma \vdash v: \tau
} {
  \Gamma \vdash \eret{v}: \tau \mid \epsilon
}
\and
\inferrule*[right=TExp-Let] {
  \Gamma \vdash e_1 : \tau_1 \mid E_1
  \and
  \Gamma, x: \tau_1 \vdash e_2 : \tau_2 \mid E_2
} {
  \Gamma \vdash \elet{x}{e_1}{e_2}: \tau_2 \mid E_1 \circ E_2
}
\and
\inferrule*[right=TExp-AppFun] {
  \Gamma \vdash v_1 : \tau_1 \xrightarrow{E} \tau_2
  \and
  \Gamma \vdash v_2 : \tau_1
} {
  \Gamma \vdash v_1\ v_2: \tau_2 \mid E
}
\and
\inferrule*[right=TExp-AppComp] {
  \Gamma \vdash v_1 : \mf{Component}\ \tau
  \and
  \Gamma \vdash v_2 : \tau
} {
  \Gamma \vdash v_1\ v_2: \mf{View} \mid \epsilon
}
\and
\inferrule*[right=TExp-AppSet] {
  \Gamma \vdash v_1 : \mf{Setter}\ \tau
  \and
  \Gamma \vdash v_2 : \tau
} {
  \Gamma \vdash v_1\ v_2: \one \mid \mf{Mut}
}
\and
\inferrule*[right=TExp-If] {
  \Gamma \vdash v : \mf{Bool}
  \and
  \Gamma \vdash e_1 : \tau \mid E_1
  \and
  \Gamma \vdash e_2 : \tau \mid E_2
} {
  \Gamma \vdash \eif{v}{e_1}{e_2} : \tau \mid E_1 \circ E_2
}
\end{mathpar}

\subsubsection{Top-Level Expression Typing}

Judgment: $\Gamma \Vdash \hat{e}: \tau$

\begin{mathpar}
\inferrule*[right=THat-Lift] {
  \Gamma \vdash e: \tau \mid \epsilon
} {
  \Gamma \Vdash \hat{e} : \tau
}
\and
\inferrule*[right=THat-Let] {
  \Gamma \vdash e: \tau_1 \mid \epsilon
  \and
  \Gamma, x: \tau_1 \Vdash \hat{e}: \tau_2
} {
  \Gamma \vdash \elet{x}{e}{\hat{e}}: \tau_2
}
\and
\inferrule*[right=THat-State] {
  \Gamma \vdash v: \tau_1
  \and
  \Gamma, x: \tau_1, x_\mf{set}: \mf{Setter}\ \tau_1
  \Vdash \hat{e} : \tau_2
} {
  \Gamma \Vdash \estate{x}{\ell}{v}{\hat{e}} : \tau_2
}
\end{mathpar}

\subsubsection{Program Typing}

A program is an expression of type $\mf{View} \mid \epsilon$.

\subsection{Semantics}

\subsubsection{Pure Expression Semantics}

Judgment: $m \semi e \mapsto e' \semi m'$

\begin{mathpar}
\inferrule*[right=E-Let] {
  m \semi e_1 \mapsto e_1' \semi m'
} {
  m \semi \elet{x}{e_1}{e_2} \mapsto \elet{x}{e_1'}{e_2} \semi m'
}
\end{mathpar}
\begin{align*}
m \semi \elet{x}{\eret{v}}{e} &\mapsto e[v/x] \semi m
&&\textsc{E-Ret}\\
m \semi (\vrec{f}{x}{\tau}{e})\  v &\mapsto e[v/x, \vrec{f}{x}{\tau}{e}/f] \semi m
&&\textsc{E-AppFun}\\
m \semi (\vcomp{\mc{C}}{x}{\tau}{e})\ v &\mapsto 
\eret{\four{\mc{C}}{v}{\tau}{x.e}} \semi m
&&\textsc{\hl{E-AppComp}}\\
m \semi \vset{\ell}{p}\ v &\mapsto \eret{()} \semi m'
&&\textsc{E-AppSet}\\
(m' &= m \mid m[p].st[\ell] = v)\\
m \semi \eif{\mf{true}}{e_1}{e_2} &\mapsto e_1 \semi m
&&\textsc{E-IfTrue}\\
m \semi \eif{\mf{false}}{e_1}{e_2} &\mapsto e_2 \semi m
&&\textsc{E-IfFalse}
\end{align*}

\subsubsection{Top-level Expression Semantics}

Judgment: $m \semi \hat{e} \topstep{\phi}{p} \hat{e}' \semi m'$

\begin{mathpar}
\inferrule*[right=E-Lift] {
  m \semi e \mapsto e' \semi m'
} {
  m \semi e \topstep{\phi}{p} e' \semi m'
}
\and
\inferrule*[right=E-LetTop] {
  m \semi e \mapsto e' \semi m'
} {
  m \semi \elet{x}{e}{\hat{e}} \topstep{\phi}{p} \elet{x}{e'}{\hat{e}} \semi m'
}
\end{mathpar}
\begin{align*}
m \semi \elet{x}{\eret{v}}{\hat{e}} &\topstep{\phi}{p} \hat{e}[v/x] \semi m
&&\textsc{E-RetTop}\\
m \semi \estate{x}{\ell}{v}{\hat{e}}
&\topstep{\mf{Init}}{p}
\hat{e}[v/x, \vset{\ell}{p}/x_\mf{set}] \semi m'
&&\textsc{E-StInit}\\
(m' &= m \mid m[p].\mf{st}[\ell] = v)\\
m \semi \estate{x}{\ell}{v}{\hat{e}}
&\topstep{\mf{Succ}}{p}
\hat{e}[v'/x, \vset{\ell}{p}/x_\mf{set}] \semi m
&&\textsc{E-StSucc}\\
(v' &= m[p].\mf{st}[\ell])
\end{align*}

\subsubsection{Initialization}

Judgment: $m \vdash \mf{init}(v) = \pair{t}{m'}$

\begin{mathpar}
\inferrule*[right=Init-Lit] { } {
  m \vdash \mf{init}(\mf{text}\ v) = \pair{\mf{lit}\ v}{m}
}
\and
\inferrule*[right=Init-Node] {
  id\ \mf{fresh}
  \and
  (0 \leq j < n)\quad m_j \vdash \mf{init}(v_j) = \pair{t_j}{m_{j+1}}
} {
  m_0 \vdash \mf{init}(\mf{tag}(v, \num{v_i}, \num{v_j})) 
  = \pair{\mf{node}(id, v, \num{v_i}, \num{t_j})}{m_n}
}
\and
\inferrule*[right=\hl{Init-Comp}] {
  m_0 \vdash p\ \mf{fresh}
  \\
  m_1 = m_0[p \mapsto \{\mf{spec}: \four{\mc{C}}{v}{\tau}{x.\hat{e}},
  \mf{st}: \emptyset, \mf{child}: \emptyset\}]
  \\
  m_1 \semi \hat{e}[v/x, \vcomp{\mc{C}}{x}{\tau}{\hat{e}}/\mc{C}] 
  \multistep{\mf{Init}}{p} \eret{v'} \semi m_2
  \\
  m_2 \vdash \mf{init}(v') = \pair{t}{m_3}
} {
  m_0 \vdash \mf{init}(\pair{\vcomp{\mc{C}}{x}{\tau}{\hat{e}}}{v}) 
  = \pair{p}{m_3 \mid m_3[p].\mf{child} = t}
}
\end{mathpar}

\pagebreak

\subsubsection{Rerendering}

Judgment: $m_\mf{old} \vdash \mf{check}(m_\mf{new}, t) = m$

\begin{mathpar}
\inferrule*[right=Check-Lit] { } {
  m_\mf{old} \vdash \mf{check}(m_\mf{new}, \mf{lit}\ v) 
  = m_\mf{new}
}
\and
\inferrule*[right=Check-Node] {
  (0 \leq j < n) \quad
  m_\mf{old} \vdash \mf{check}(m_j, t_j) = \pair{t_j'}{m_{j+1}}
} {
  m_\mf{old} \vdash \mf{check}(m_0, \mf{node}(id, v, \num{v_i}, \num{t_j})) = m_n
}
\and
\inferrule*[right=Check-PathConst] {
  m_\mf{old}[p].\mf{st} = m_\mf{new}[p].\mf{st}\\
  m_\mf{old} \vdash \mf{check}(m_\mf{new}, m_\mf{old}[p].child) 
  = m
} {
  m_\mf{old} \vdash \mf{check}(m_\mf{new}, p) 
  = m
}
\and
\inferrule*[right=\hl{Check-PathDiff}] { 
  m_\mf{old}[p].\mf{st} \neq m_\mf{new}[p].\mf{st}
  \\
  m'[p].\mf{spec} = \four{\mc{C}}{v}{\tau}{x.\hat{e}}
  \\
  m_\mf{new} \semi \hat{e}[v/x, \vcomp{\mc{C}}{x}{\tau}{\hat{e}}/\mc{C}]
   \multistep{\mf{Succ}}{p} \eret{v'} \semi m
  \\
  m \vdash \mf{init}(v') = \pair{t}{m'}
} {
  m_\mf{old} \vdash \mf{check}(m_\mf{new}, p)
   = m' \mid m[p].\mf{child}=t
}
\end{mathpar}

\subsubsection{Handler Search}

Judgment: $m \vdash \mf{handlers}(t, id) = \num{v}$

\begin{mathpar}
\inferrule*[right=Handlers-Lit] { } {
  m \vdash \mf{handlers}(\mf{lit}\ v, id) = \{\}
}
\and
\inferrule*[right=Handlers-Comp] {
  m \vdash \mf{handlers}(m[p].\mf{child}, id) = \num{v}
} {
  m \vdash \mf{handlers}(p, id) = \num{v}
}
\and
\inferrule*[right=Handlers-Tgt] { } {
  m \vdash \mf{handlers}(\mf{node}(id, v, \num{v_i}, \num{v_j}), id)
  = \{v_h \mid \mf{onClick}\ v_h \in \num{v_i}\}
}
\and
\inferrule*[right=Handlers-Node] {
  id \neq id'
  \and
  (\forall j)\ 
  m \vdash \mf{handlers}(v_j, id) = h_j
} {
  m \vdash \mf{handlers}(\mf{node}(id', v, \num{v_i}, \num{v_j}), id)
  = \bigcup_{\forall j} h_j
}
\end{mathpar}

\subsubsection{Event Loop}

Judgment: $\trip{m}{t}{\mu} \hookrightarrow \trip{m'}{t}{\mu'}$

\begin{mathpar}
\inferrule*[right=Loop-Start] {
  \cdot \semi e \mapsto^* \eret{v} \semi m
  \\
  m \vdash \mf{init}(v) = \pair{t}{m'}
} {
  \trip{\cdot}{\cdot}{\mf{start}(e)} 
  \hookrightarrow
  \trip{m'}{t}{\mf{idle}}
}
\and
\inferrule*[right=Loop-Event] {
  \exists\; id
  \and
  m \vdash \mf{handlers}(t, id) = \num{v_i}
  \\
  (0 \leq i < n)\quad
  m_i \semi v_i\ () \mapsto^* \eret{()} \semi m_{i+1}
} {
  \trip{m_0}{t}{\mf{idle}} \hookrightarrow \trip{m}{t}{\mf{rerender}(m_n)}
}
\and
\inferrule*[right=Loop-Rerender] {
  m_\mf{old} \vdash \mf{check}(m_\mf{new}, t) = m
} {
  \trip{m_\mf{old}}{t}{\mf{rerender}(m_\mf{new})}
  \hookrightarrow
  \trip{m}{t}{\mf{idle}}
}
\end{mathpar}

\end{document}