...
we have $\Gamma \semi M \vdashv m[p].\mf{st}(\ell): M[\ell]$.

Also define $\Gamma \sqsubseteq \Gamma'$ to hold when,
for all $x \in \Gamma$, both $x \in \Gamma'$ and $\Gamma[x] = \Gamma'[x]$.
...
\begin{theorem}[Progress of top-level expressions, initial phase]
If $\cdot \semi M \Vdashd \hat{e}: \tau$, then either:
\begin{enumerate}[(i)]
\item $\hat{e} = \eret{v}$, or
\item for all $m$ such that $\cdot \semi M \vDash m$, 
there exist some $\hat{e}'$ and $m'$ such that 
$m \semi \hat{e} \mapsto^\mf{Init}_p \hat{e}' \semi m'$
(for any $p$).
\end{enumerate}
\end{theorem}
...
$m \semi \hat{e} \smallstep{\mf{Init}}{p} \hat{e}' \semi m'$.
...
\smallstep{\mf{Init}}{p} \hat{e}[v/x] \semi m$
and we are done.

If instead $m \semi e \mapsto e' \semi m'$, then we apply \textsc{E-LetTop}
to get $m \semi \elet{x}{e}{\hat{e}} 
\smallstep{\mf{Init}}{p} \elet{x}{e'}{\hat{e}} \semi m'$ and we are done.
...
To prove progress in the $\mf{Succ}$ phase, we have to do a bit more work.

Define $\mc{L} ::= \map{\mc{C}}{\{\ell\}}$, i.e.
$\mc{L}$ maps component names to the set of labels contained in its body.
This can be determined statically, like $\Delta$.

Also define $m \vdash p\ \mf{initializes}\ \mc{C}$
to say that $m[p].\mf{spec} = \pair{\mc{C}}{v_0}$ and,
for all $\ell \in \mc{L}[\mc{C}]$,
there exists some $v$ such that $m[p].\mf{st}[\ell] = v$.

\begin{theorem}[Progress of top-level expressions, successor phase]
If:
\begin{enumerate}[(i)]
\item $\cdot \semi M \Vdashd \hat{e}: \tau$,
\item $\cdot \semi M \vDash m$,
\item $m \vdash p\ \mf{initializes}\ \mc{C}$, and
\item $\mf{dom}(M) \subseteq \mc{L}[\mc{C}]$,
\end{enumerate}
then either:
\begin{enumerate}
\item $\hat{e} = \eret{v}$, or
\item there exist some $\hat{e}'$ and $m'$ such that 
$m \semi \hat{e} \mapsto^\mf{Succ}_p \hat{e}' \semi m'$.
\end{enumerate}
\end{theorem}
...
\textbf{Case} \textsc{THat-State}:
We have\\
$\cdot \semi M \Vdashd \estate{x}{\ell}{v}{\hat{e}}$,
which gives that $\ell \in \mf{dom}(M)$ by inversion.
By \emph{(iv)}, $\ell \in \mc{L}[\mc{C}]$,
and by \emph{(iii)}, $m[p].\mf{st}[\ell]$ is defined;
call this value $v'$.
Therefore, we can apply \textsc{E-StSucc} to get
$m \semi \estate{x}{\ell}{v}{\hat{e}} \smallstep{\mf{Succ}}{p}
\hat{e}[v'/x, \vset{\ell}{p}/x_\mf{set}] \semi m$,
and we are done.

Other cases are identical to the initial stage.
...
\end{enumerate}
then $\Gamma \semi M \vDash m'$ and $\Gamma \semi M \vdashd e': \tau \mid E$.
\end{theorem}
...
By inductive hypothesis, $\Gamma \semi M \vDash m'$
...
The preserved type $\one \mid \mf{Mut}$ is easy to show.
It remains to be shown that 
$\Gamma \semi M \vDash m \mid m[p].\mf{st}[\ell] = v$.
This is given by \emph{(ii)} for all $\ell' \neq \ell$;
we want to show $\Gamma \semi M \vdashd v : M[\ell]$.
...
and by preservation of expressions, 
$\Gamma \semi M \vdashd e': \tau \mid \epsilon$.
...
By preservation of expressions,
$\Gamma \semi M \vdashd e': \tau' \mid \epsilon$.
...
We first want to show that $\Gamma \semi M \vDash m'$.
It follows from \emph{(ii)} that we only need to show 
$\Gamma \semi M \vdashd m'[\ell] : M[\ell]$.
This is given by $M[\ell] = \tau_1$
and $\Gamma \semi M \vdashd v: \tau$.

It remains to be shown that 
$\Gamma \semi M \Vdashd \hat{e}[v/x, \vset{\ell}{p}/x_\mf{set}]: \tau$.
This follows from the substitution lemma applied twice
to the final premise of \emph{(i)}.
...
and by \emph{(i)}, we also know $M[\ell] = \tau_1$.
We can therefore apply the substitution lemma twice
to get that 
$\Gamma \semi M \Vdashd \hat{e}[v'/x, \vset{\ell}{p}/x_\mf{set}] : \tau_1$
\end{proof}
...
\end{enumerate}
then $\Gamma \semi M \vDash m'$ and $\Gamma \semi M \Vdashd P$.
\end{theorem}
