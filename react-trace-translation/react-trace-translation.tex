...
We introduce the additional context $M=\map{\ell}{\tau}$
to type the tree memory.

Define $\Gamma \semi M \vDash m$ to say that 
$\ell \in M$ iff, for all $p$ such that $m[p].\mf{st}(\ell)$ is defined,
we have $\Gamma \semi M \vdashv m[p].\mf{st}(\ell): M[(p, \ell)]$.

Further define $M \sqsubseteq M'$ to hold when,
for all $\ell \in M$, both $\ell \in M'$ and $M[\ell] = M'[\ell]$.
...
and programs with $M$ analogously; these extensions are 
mostly straightforward
and thus elided.

% The one exception is $\mf{useState}$, which gains an extra premise on $M$:
% \begin{mathpar}
% \inferrule*[right=\hl{THat-State}] {
%   \ell: \tau_1 \in M
%   \and
%   \Gamma \semi M \vdashd v: \tau_1
%   \\
%   \Gamma, x: \tau_1, x_\mf{set}: \mf{Setter}\ \tau_1 \semi M \Vdashd \hat{e}: \tau_2
% } {
%   \Gamma \semi M \Vdashd \estate{x}{\ell}{v}{\hat{e}}: \tau_2
% }
% \end{mathpar}
...
If $\cdot \semi M \vdashv v: \tau$, then
\begin{itemize}
...
Substitution lemmas for top-level expressions and programs are necessary,
but we elide their (standard) proofs.
...
and by canonical forms, $v_1=\mc{C}$.
We apply \textsc{E-AppComp} to get 
...
We apply \textsc{E-AppSet} to get
...
\textbf{Case} \textsc{THat-Lift}:
We have $\cdot \semi M \Vdashd \hat{e}: \tau$,
and by inversion, $\cdot \semi M \vdashd e: \tau \mid \epsilon$.
By progress of expressions, $m \semi e \vdash e' \semi m'$ 
for an appropriate $m$.
We conclude by \textsc{E-Lift} that 
$m \semi \hat{e} \smallstep{\phi}{p} \hat{e}' \semi m'$.

\textbf{Case} \textsc{THat-Let}:
We have $\cdot \semi M \Vdashd \elet{x}{e}{\hat{e}}: \tau_2$,
and by inversion, $\cdot \semi M \vdashd e: \tau_1 \mid \epsilon$.
By progress of expressions, we have two cases:
either $e = \eret{v}$ or else $m \semi e \mapsto e' \semi m'$
for an appropriate $m$.

If $e = \eret{v}$, then we apply \textsc{E-RetTop}
to get $m \semi \elet{x}{\eret{v}}{\hat{e}} 
\smallstep{\phi}{p} \hat{e}[v/x] \semi m$
and we are done.

If instead $m \semi e \mapsto e' \semi m'$, then we apply \textsc{E-LetTop}
to get $m \semi \elet{x}{e}{\hat{e}} 
\smallstep{\phi}{p} \elet{x}{e'}{\hat{e}} \semi m'$ and we are done.

\textbf{Case} \textsc{THat-State}:
We have $\cdot \semi M \Vdashd \estate{x}{\ell}{v}{\hat{e}}$.
We subcase on the value of our arbitrary $\phi$.

If $\phi = \mf{Init}$, then \textsc{E-StInit}
gives $m \semi \estate{x}{\ell}{v}{\hat{e}}
\smallstep{\mf{Init}}{p} \hat{e}[v/x, \vset{\ell}{p}/x_\mf{set}] \semi m'$
(where $m' = m \mid m[p].\mf{st}[\ell] = v$) and we are done.

If $\phi = \mf{Succ}$, then \textsc{E-StSucc}
gives $m \semi \estate{x}{\ell}{v}{\hat{e}}
\smallstep{\mf{Succ}}{p} \hat{e}[v'/x, \vset{\ell}{p}/x_\mf{set}] \semi m'$
(where $v' = m[p].\mf{st}[\ell]$).

\hl{It remains to be shown that $v'$ is defined in the above subcase.}
Intuitively, if $\phi=\mf{Succ}$, then we must have initialized
$m[p].\mf{st}[\ell]$ in a prior $\mf{Init}$ phase, but this is not
currently captured in the judgment.

\end{proof}

\begin{theorem}[Progress of programs]
If $\cdot \semi M \vdashd P$, then either:
\begin{enumerate}[(i)]
\item $P = \erun{(\eret{v})}$, or
\item for all $\mc{D}$ and $m$ such that $\cdot \semi M \vDash m$,
there exist some $\hat{e}'$, $\mc{D}'$, and $m'$ such that 
$m \semi \mc{D} \semi \hat{e} \mapsto^\phi_p \hat{e}' \semi \mc{D}' \semi m'$.
\end{enumerate}
\end{theorem}

\begin{proof}
By induction over the typing derivation.
Straightforward, and relies on progress of expressions.
Note that $\mc{D}$ is not used, and so can be arbitrary (for now...)
\end{proof}
...
If
\begin{enumerate}[(i)]
\item $\Gamma \semi M \vdashd e: \tau \mid E$,
\item $\Gamma \semi M \vDash m$ for some $m$, and
\item $m \semi e \mapsto e' \semi m'$ for some $e'$ and $m'$,
\end{enumerate}
then there is some $M'$ such that:
\begin{itemize}
\item $M \sqsubseteq M'$,
\item $\Gamma \semi M' \vDash m'$, and
\item $\Gamma \semi M' \vdashd e': \tau \mid E$.
\end{itemize}
\end{theorem}
...
\textbf{Case} \textsc{E-LetPure}:
We have $m \semi \elet{x}{e_1}{e_2} \mapsto \elet{x}{e_1'}{e_2} \semi m'$.
By inversion on \textsc{E-Let}, we also have $m \semi e_1 \mapsto e_1' \semi m'$.
By inversion on \emph{(i)}, $\Gamma \semi M \vdashd e_1: \tau_1 \mid E_1$.
By inductive hypothesis, there exists some $M'$
with $\Gamma \semi M' \vDash m'$
and $\Gamma \semi M' \vdashd e_1: \tau_1 \mid E_1$.
Since $M \sqsubseteq M'$, and $\Gamma, x: \tau_1 \semi M \vdashd e_2: \tau \mid E_2$ (by inversion),
we also get that $\Gamma, x: \tau_1 \semi M' \vdashd e_2: \tau \mid E_2$.
We apply \textsc{E-Let} to get $\Gamma \semi M' \vdashd \elet{x}{e_1'}{e_2}: \tau \mid E$
(for $E = E_1 \circ E_2$).

\textbf{Case} \textsc{E-Ret}:
We have $m \semi \elet{x}{\eret{v}}{e} \mapsto e[v/x] \semi m$.
Inversion on \emph{(i)} gives that $\Gamma \semi M \vdashd v: \tau_1$ and
$\Gamma, x: \tau_1 \semi M \vdashd e: \tau \mid E$. We conclude
$\Gamma \semi M \vdashe e[v/x]: \tau \mid E$ via the substitution lemma.

\textbf{Case} \textsc{E-AppFun}:
We have $m \semi (\lambda x: \tau_1.e)v \mapsto e[v/x] \semi m$.
Inversion on \emph{(i)} gives $\Gamma, x: \tau_1 \semi M \vdashe e: \tau \mid E$
and $\Gamma \semi M \vdashv v: \tau_1$. We again get
$\Gamma \semi M \vdashe e[v/x]: \tau \mid E$ via the substitution lemma.

\textbf{Case} \textsc{E-AppComp}:
We have $m \semi \mc{C} v \mapsto \eret{\pair{\mc{C}}{v}} \semi m$.
Both sides have type $\mf{View} \mid \epsilon$: the left-hand side
follows from \textsc{TExp-AppComp}, and the right from \textsc{TExp-Ret}
and \textsc{TVal-Spec}.

\textbf{Case} \textsc{E-AppSet}:
We have $m \semi \vset{\ell}{p}\ v \mapsto 
\eret{()} \semi m \mid m[p].\mf{st}[\ell] = v$.
Let $M'=M$; that $M' \sqsubseteq M$ is obvious,
and the preserved type $\one \mid \mf{Mut}$ is easy to show.
It remains to be shown that 
$\Gamma \semi M \vDash m \mid m[p].\mf{st}[\ell] = v$.
This is given by \emph{(ii)} for all $\ell' \neq \ell$;
we want to show $\Gamma \semi v : M[\ell]$
By inversion on \emph{(i)}, $\Gamma \semi M \vdashd v: \tau$
and $\ell: \tau \in M$.

\textbf{Case} \textsc{E-IfTrue}:
We have $m \semi \eif{\mf{true}}{e_1}{e_2} \mapsto e_1 \semi m$.
Immediate by inversion on \emph{(i)}. Case for \textsc{E-IfFalse} is analogous.
\end{proof}

\begin{theorem}[Preservation of top-level expressions]
If
\begin{enumerate}[(i)]
\item $\Gamma \semi M \Vdashd \hat{e}: \tau$,
\item $\Gamma \semi M \vDash m$ for some $m$, and
\item $m \semi \hat{e} \mapsto^{\phi}_p \hat{e}' \semi m'$ 
for some $\hat{e}'$, $m'$ and arbitary $\phi$, $p$,
\end{enumerate}
then there is some $M'$ such that:
\begin{itemize}
\item $M \sqsubseteq M'$,
...
\textbf{Case} \textsc{E-Lift}:
We have that $m \semi \hat{e} \smallstep{\phi}{p} \hat{e}' \semi m'$.
By inversion, $m \semi e \mapsto e' \semi m'$.
Inversion on \emph{(i)} gives that
$\Gamma \semi M \vdashd e: \tau \mid \epsilon$,
and by preservation of expressions, $\Gamma \semi M' \vdashd e': \tau \mid \epsilon$
for some $M' \sqsubseteq M$ with $\Gamma \semi M' \vdashv m'$.
Reapplication of \textsc{E-Lift} gives the desired conclusion,
$\Gamma \semi M' \Vdashd \hat{e}': \tau$.

\textbf{Case} \textsc{E-LetTop}:
We have that $m \semi \elet{x}{e}{\hat{e}} 
\smallstep{\phi}{p} \elet{x}{e'}{\hat{e}} \semi m'$,
and by inversion, $m \semi e \mapsto e' \semi m'$.
Inversion on \emph{(i)} gives that
$\Gamma \semi M \vdashd e: \tau' \mid \epsilon$
and $\Gamma, x: \tau' \semi M \Vdashd \hat{e}: \tau$.
By preservation of expressions, $\Gamma \semi M' \vdashd e': \tau' \mid \epsilon$
for some $M'$ with $M \sqsubseteq M'$ and $\Gamma \semi M' \vDash m'$.
Since $M \sqsubseteq M'$, we can also say that
$\Gamma, x: \tau' \semi M' \Vdashd \hat{e}: \tau$, and then
reapplication of \textsc{E-LetTop} gives the desired conclusion,
$\Gamma \semi M' \Vdashd \elet{x}{e'}{\hat{e}'}: \tau$.

\textbf{Case} \textsc{E-RetTop}:
We have $m \semi \elet{x}{\eret{v}}{\hat{e}} \smallstep{\phi}{p} \hat{e}[v/x] \semi m$.
Inversion on \emph{(i)} gives that
$\Gamma \semi M \vdashd v: \tau'$
and $\Gamma, x: \tau' \semi M \Vdashd \hat{e}: \tau$.
The conclusion follows from the substitution lemma.

\textbf{Case} \textsc{E-StInit}:
We have\\
$m \semi \estate{x}{\ell}{v}{\hat{e}}
\smallstep{\mf{Init}}{p}
\hat{e}[v/x, \vset{\ell}{p}/x_\mf{set}] \semi m'$,
where $m' = m | m[p].\mf{st}[\ell] = v$.
By inversion on \emph{(i)}, we get
$\Gamma \semi M \vdashd v: \tau_1$ and
$\Gamma, x: \tau_1, x_\mf{set}: \mf{Setter}\ \tau_1 \semi M \Vdashd \hat{e}: \tau$.

Let $M' = M[\ell \mapsto \tau_1]$.
Clearly $\Gamma \semi M' \vDash m'$ for all $\ell' \neq \ell$;
that it is satisfied for $\ell$ follows from that 
$M'[\ell] = \tau_1$ and $\Gamma \semi M' \vdashd v: \tau_1$.
That $\Gamma \semi M' \Vdashd \hat{e}[v/x, \vset{\ell}{p}/x_\mf{set}]$
follows quickly from the substitution lemma (if $M \sqsubseteq M'$).

\hl{However, we cannot conclude that $M \sqsubseteq M'$,}
since we do not know that $\ell: \tau' \notin M$.
Intuitively, that we are in phase $\mf{Init}$
and that $\ell$ is unique
guarantees that $\ell \notin \mf{dom}(M)$,
but this is not captured by the judgment nor an assumption.

\textbf{Case} \textsc{E-StSucc}:
We have\\
$m \semi \estate{x}{\ell}{v}{\hat{e}}
\smallstep{\mf{Succ}}{p}
\hat{e}[v'/x, \vset{\ell}{p}/x_\mf{set}] \semi m$,
where $v' = m[p].\mf{st}[\ell]$.
By inversion on \emph{(i)},
we know that $\Gamma, x: \tau_1, x_\mf{set}: \mf{Setter}\ \tau_1
\semi M \Vdashd \hat{e}: \tau$.
It follows by definition that
$\Gamma \semi M \vdashv \vset{\ell}{p} : \mf{Setter}\ \tau_1$.
% We can thus apply the substitution lemma to get that
% $\Gamma \semi M \Vdashd \hat{e}[v'/x, \vset{\ell}{p}/x_\mf{set}] : \tau$.

By \emph{(ii)}, we know $\Gamma \semi M \vdashd v': M[\ell]$.
However, we cannot quite apply the substitution lemma since
\hl{we do not know that $M[\ell] = \tau_1$}.
Intuitively, $v'$ must have the same type as the original
state value $v$, but this relationship escapes the current formalization.
\end{proof}

\begin{theorem}[Preservation of programs]
If
\begin{enumerate}[(i)]
\item $\Gamma \semi M \vdashd P$,
\item $\Gamma \semi M \vDash m$ for some $m$, and
\item $m \semi \mc{D} \semi P \Mapsto P \semi \mc{D}' \semi m'$ 
for some $P'$, $\mc{D}'$, and $m'$,
\end{enumerate}
then there is some $M'$ such that:
\begin{itemize}
\item $M \sqsubseteq M'$,
...
\begin{proof}
By induction on \emph{(iii)}; standard. We again ignore $\mc{D}$.
\end{proof}
...
NOTE: $\mf{let}\ \mf{rec}$ should be replaced 
with e.g. anonymous recursive functions;
current substitution semantics are incorrect for FGCBV
